...
Sequence-level `fuse_seq` RoPE-attends every left token over all right tokens — pooled fused ≠ fuse(last). GameForge BPE is the other mouth (word tokens stay Jaccard). `evaluate()` is a gate: trained ppl / mix_mae / gate-entropy / bpe-compression must beat untrained. Hive `pull` replaces experts iff merkle differs. Bias/policy heads author the plan the same way mix does — only if slack rises.

Zaibatsu mouth: RoPE lives in the residual Q/K (order is not a bag). FFN is GELU (negative mass ReLU zeros). KV-cache decode matches full forward. Greedy = argmax. PFC drafts, neo verifies (speculate). `tournament()` names a winner among PFC / midbrain / neo; merkle is the family seal; devil gene is own-lm armed. GPU harness applies the same RoPE+GELU (tanh approx, not erf). CPU is native. `to("cuda")` still degrades without a GPU.
