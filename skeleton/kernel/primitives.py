...
class DomainEvent:
    ...
    ``event_id`` and ``causation_id`` are excluded from dataclass equality so
    existing callers that compare events by their original payload contract do
    not change behavior. They provide stable causal identity for replay and
    tracing without breaking the pre-existing constructor surface.
    """
    ...
    def occurred_at(self) -> float:
        """Compatibility alias used by causal-graph consumers."""
    ...
    def derive(
        ...
        *,
        ...
        """Create a child event that preserves this event's causal context."""
...
class EventBus:
    ...
    A bounded replay buffer enables diagnostics and causal reconstruction while
    preventing event history from becoming an unbounded process-lifetime leak.
    """
    ...
    def publish(self, event: DomainEvent) -> DomainEvent:
        """Publish an event to all matching subscribers and retain it for replay."""
