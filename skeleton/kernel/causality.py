...
The graph is intentionally pure domain logic: it performs no I/O and reads no
clock. It reconstructs cause -> effect chains from ``DomainEvent`` identity,
correlation and causation metadata retained by ``EventBus``.
"""
...
class CausalNode:
    """Resolved place of one event in a causal graph."""
...
@dataclass(frozen=True)
class CausalPath:
    """Ordered chain of events from the nearest known root to one event."""
...
class CausalGraph:
    """Directed cause -> effect graph over correlated ``DomainEvent`` objects.

    Ingestion is idempotent by ``event_id`` and accepts out-of-order streams.
    A child whose parent has not arrived yet is temporarily exposed as a root;
    when the parent later arrives the edge is grafted automatically.
    """
    ...
    def add(self, event: DomainEvent) -> None:
        """Ingest one event, ignoring duplicate ``event_id`` values."""
    ...
    def add_all(self, events: Iterable[DomainEvent]) -> int:
        """Ingest many events and return the count of new events."""
    ...
    def from_bus(cls, bus: Any, correlation_id: str | None = None) -> "CausalGraph":
        """Build a graph from the bounded replay window of an ``EventBus``."""
    ...
    def node(self, event_id: str) -> CausalNode:
        """Return one node with its currently resolved parent, children and depth."""
    ...
    def frontier(self) -> list[CausalNode]:
        """Return the live edge of the graph."""
    ...
    def lineage(self, event_id: str) -> CausalPath:
        """Return the nearest known root -> ``event_id`` causal chain."""
    ...
    def fan_out(self, event_id: str | None = None) -> dict[str, int] | int:
        """Return direct child count for one event or every retained event."""
    ...
    def cycles(self) -> list[list[str]]:
        """Return causal loops; a healthy causal graph normally returns none."""
    ...
    def stats(self) -> dict[str, int]:
        """Return a stable shape summary of the graph."""
    ...
    def walk(self, from_event_id: str | None = None) -> Iterator[CausalNode]:
        """Breadth-first traversal from one event or every resolved root."""
