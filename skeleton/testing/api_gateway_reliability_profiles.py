...
def run_api_gateway_throughput_profile(*, requests: int = 5_000, concurrency: int = 32) -> APIProfileResult:
    ...
    def one_request(index: int) -> tuple[int, bool, float]:
        ...
        return response.status, response.body == {"index": index}, response.duration_ms
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run concurrent API-gateway reliability pressure.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = run_api_gateway_throughput_profile(requests=args.requests, concurrency=args.concurrency)
