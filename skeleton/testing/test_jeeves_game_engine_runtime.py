...
def test_unified_runtime_rejects_and_repairs_common_manifest_corruption() -> None:
    lab = ExecutableGameEngineLab()
    sandbox = lab.create(
        EngineEra.PONG,
        "arcade_golden_age",
    ...
    assert not before.passed
    assert "base:manifest" in before.failed
    assert "contract" in before.failed
    assert path in before.contract_mismatches
...
def test_unified_runtime_repair_removes_unexpected_engine_control_file() -> None:
    lab = ExecutableGameEngineLab()
    sandbox = lab.create(
        EngineEra.FIXED_3D
    )
    rogue_path = (
        "engine/rogue_control.json"
    )
    broken = sandbox.apply(
        (
            SandboxPatch(
    ...
    assert "contract" in report.failed
    assert (
        rogue_path
        in report.contract_mismatches
    )
...
def test_runtime_fix_cannot_hitchhike_generated_control_file_regression() -> None:
    lab = ExecutableGameEngineLab()
    sandbox = lab.create(
        EngineEra.EIGHT_BIT
    )
...
def test_engine_evolution_rejects_unattested_path_even_with_real_repair() -> None:
    lab = ExecutableGameEngineLab()
    sandbox = lab.create(
        EngineEra.MODERN
    )
