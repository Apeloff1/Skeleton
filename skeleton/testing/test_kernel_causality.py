...
def test_missing_parent_remains_safe_root() -> None:
    orphan = DomainEvent(topic="orphan", correlation_id="corr", causation_id="not-retained")
    graph = CausalGraph()
    graph.add(orphan)
    assert graph.node(orphan.event_id).is_root
    assert graph.node(orphan.event_id).depth == 0
