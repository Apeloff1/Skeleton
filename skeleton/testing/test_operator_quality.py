...
def test_product_card_exposes_quality(tmp_path):
    ...
    append_repair({"surface": "forge", "ok": 1, "before": {"reason": "low_score"}, "after": {"reason": "accepted", "score": 0.8, "weakest_path": "world_map.gd"}, "actions": [{"path": "world_map.gd"}]}, root=tmp_path)
...
def test_nervous_card_exposes_quality_pressure(tmp_path):
    ...
    append_repair({"surface": "plan", "ok": 1, "before": {"reason": "low_score"}, "after": {"reason": "accepted", "score": 0.9, "weakest_path": "grounding"}, "actions": [{"field": "era"}]}, root=tmp_path)
...
def test_doctor_card_exposes_quality(tmp_path):
    ...
    append_repair({"surface": "forge", "ok": 0, "before": {"reason": "project_closure"}, "after": {"reason": "low_score", "score": 0.4, "weakest_path": "project.godot"}, "actions": [{"path": "project.godot"}]}, root=tmp_path)
    org = Organismer(root=tmp_path, persist=False, galaxy=GalaxySystem())
...
def test_satellites_card_exposes_quality(tmp_path):
    ...
    append_repair({"surface": "forge", "ok": 1, "before": {"reason": "unsafe_code"}, "after": {"reason": "accepted", "score": 0.85, "weakest_path": "world_map.gd"}, "actions": [{"path": "world_map.gd"}]}, root=tmp_path)
    org = Organismer(root=tmp_path, persist=False, galaxy=GalaxySystem())
