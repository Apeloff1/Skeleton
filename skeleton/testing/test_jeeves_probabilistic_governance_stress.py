...
def _horizon_report(*, horizons: int = 4, count: int = 40):
    observations = tuple(
        StratifiedForecastObservation(
            target_index=index,
            actual=0.5 + 0.1 * float(index % 3),
            predictive=_Forecast(horizon=1 + (index - 1) % horizons),
        )
        for index in range(1, count + 1)
    )
    return evaluate_stratified_conformal(
...
def test_many_thin_horizons_are_visible_as_unevidenced_horizon_mass() -> None:
    report = _horizon_report(horizons=4, count=40)
    permissive = evaluate_conformal_governance(report, gate=_gate())
    constrained = evaluate_conformal_governance(
        report,
        gate=_gate(
            min_horizon_steps=7,
            max_unevidenced_horizon_step_rate=0.10,
        ),
    )

    assert permissive.observed_horizons == 4
    assert permissive.evidenced_horizons == 4
    assert constrained.unevidenced_horizon_step_rate > 0.10
    assert not constrained.eligible
    assert "conformal_unevidenced_horizon_step_rate_above_gate" in constrained.reasons


def test_horizon_coverage_is_aggregated_independently_of_bucket_identity() -> None:
    report = _horizon_report(horizons=4, count=40)
    decision = evaluate_conformal_governance(
        report,
        gate=_gate(max_horizon_coverage_gap=0.0),
    )

    assert decision.observed_horizons == 4
    assert decision.worst_horizon_coverage_gap >= 0.0
    if decision.worst_horizon_coverage_gap > 0.0:
        assert not decision.eligible
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"max_unevidenced_horizon_step_rate": -0.1},
