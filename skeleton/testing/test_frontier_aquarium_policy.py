...
def test_tank_purchase_checks_level_and_wallet_without_mutation():
    ...
    quote = quote_tank_purchase(state, tank, user_level=10, wallet={"coins": 4999})
    assert quote.affordable is False
    quote = quote_tank_purchase(state, tank, user_level=10, wallet={"coins": 5000})
    assert quote.affordable is True
    purchased = purchase_tank(state, tank)
    assert "medium" in purchased.owned_tanks
    assert "medium" in purchased.tanks
...
def test_duplicate_decoration_inventory_consumes_exactly_one():
    ...
    assert len(placed.aquarium.tanks["starter"].decorations) == 1


def test_duplicate_placed_identity_is_rejected():
    starter = AquariumTankSpec("starter", "Starter", 10, 1, 3)
    decoration = decoration_from_record(
        {"id": "seaweed", "name": "Seaweed", "category": "plant", "cost": {"coins": 100}}
    )
    state = purchase_decoration(initial_aquarium(), decoration, quantity=2)
    first = place_decoration(
        state,
        starter,
        decoration,
        position=AquariumPosition(10, 20),
        now=datetime(2026, 9, 15, 12, 0, tzinfo=timezone.utc),
        identity=lambda: "same",
    )
    with pytest.raises(ValueError, match="identity already exists"):
        place_decoration(
            ...
            starter,
            decoration,
            position=AquariumPosition(20, 30),
...
def test_total_count_mismatch_fails_closed():
    with pytest.raises(ValueError, match="must match"):
        AquariumState(
            tanks={"starter": AquariumTankState("starter", fish=(_fish("fish-1"),))},
