...
def test_nuance_update_records_governed_forecast_fusion() -> None:
    ...
    diagnostics = runtime.frame_diagnostics(frame.frame_id)
    assert diagnostics["frame_id"] == frame.frame_id
    assert diagnostics["forecast_count"] >= 2
    assert diagnostics["forecast_fusions"]
    assert diagnostics["hypergraph"] is not None
    assert diagnostics["perpendicular"] is not None
    assert (
        diagnostics["invariants"]["fusion_is_target_identity_bounded"]
        is True
    )
    assert (
        diagnostics["invariants"]["hypergraph_cannot_promote_evidence"]
        is True
    )
