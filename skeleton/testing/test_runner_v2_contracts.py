...
def test_runner_policy_authority_flags_require_booleans(field, value):
    with pytest.raises(ValueError, match="must be a boolean"):
        replace(runner_policy(), **{field: value})
...
def test_run_budget_rejects_fractional_limits_and_usage(field):
    kwargs = dict(
        request_limit=10,
        graphql_limit=10,
        mutation_limit=10,
