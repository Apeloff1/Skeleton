...
@pytest.mark.parametrize(
    "state,expected",
    [
        (MutationState.APPLIED, True),
        (MutationState.REJECTED, True),
        (MutationState.UNCERTAIN, True),
        (MutationState.ABORTED, False),
        (MutationState.FAILED, False),
        (MutationState.DUPLICATE, False),
    ],
)
def test_engine_mutation_budget_tracks_dispatched_authority_only(state, expected):
    assert _mutation_uses_budget(state) is expected
