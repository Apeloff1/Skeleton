...
from scripts.benchmark_frontier_breeding import SOURCE_SEMANTICS, run_benchmark


def test_breeding_benchmark_records_correctness_without_latency_gate():
    report = run_benchmark(iterations=64)
    assert report["workload"] == "frontier-breeding-policy-v1"
    assert report["source_semantics"] == SOURCE_SEMANTICS
    assert report["configuration"]["iterations"] == 64
    assert report["evidence"]["total_bred"] == 64
...
def test_breeding_benchmark_rejects_empty_workload():
