...
from skeleton.pr_automation.runner_transport import GitHubHTTPError
...
def test_merge_transaction_request_budget_exhaustion_is_pre_dispatch_abort(tmp_path):
    transport, index, transaction = transaction_fixture(tmp_path)
    transaction.policy = replace(
        transaction.policy,
        limits=replace(transaction.policy.limits, max_requests=1),
    )

    result = transaction.apply(work_item())

    assert result.state is MutationState.ABORTED
    assert result.message == "mutation request budget exhausted before dispatch"
    assert mutation_retriable(result) is True
    assert transport.put_calls == []
...
def test_merge_transaction_http_422_is_definitive_rejection(tmp_path):
    transport, index, transaction = transaction_fixture(tmp_path)
    transport.put_map[
        "/repos/Apeloff1/Skeleton/pulls/42/merge"
    ] = GitHubHTTPError(
...
def test_merge_transaction_ambiguous_dispatch_is_terminal_until_reconciliation(tmp_path):
    transport, index, transaction = transaction_fixture(tmp_path)
    item = work_item()
    transport.put_map[
        "/repos/Apeloff1/Skeleton/pulls/42/merge"
    ...
    assert first.state is MutationState.UNCERTAIN
    assert "outcome uncertain" in first.message
    assert mutation_retriable(first) is False
...
def test_runner_policy_can_explicitly_allow_unprotected_base():
    policy = runner_policy_from_env(
        {
            "PR_AUTOMATION_MODE": "observe",
            "PR_AUTOMATION_REQUIRED_CHECKS": "Merge Readiness",
            "PR_RUNNER_REQUIRE_PROTECTED_BASE": "false",
            "PR_RUNNER_REQUIRE_EXACT_BASE_HEAD": "true",
        }
    )
    assert policy.protected_base_required is False
    assert policy.exact_base_head_required is True
