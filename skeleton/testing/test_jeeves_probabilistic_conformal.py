...
def test_non_monotonic_target_indices_fail_closed() -> None:
    observations = (
...
def test_cross_family_adapter_rejects_malformed_report() -> None:
    with pytest.raises(StateSpaceError):
...
def test_observation_rejects_nonfinite_actual_and_invalid_forecast() -> None:
    with pytest.raises(StateSpaceError):
