...
def load_state(path: Path) -> tuple[list[Mapping[str, Any]], list[Mapping[str, Any]], list[Mapping[str, Any]], str, str | None]:
...
def propose(state_path: Path, package_path: Path, audit_path: Path, report_path: Path, config: StudioConfig) -> int:
    ...
    events.append({"ts": datetime.now(timezone.utc).isoformat(), "event": "state", "runs": len(runs), "issues": len(issues), "pulls": len(pulls)})
            ...
            if not tasks or capacity <= 0 or limit <= 0:
                ...
                if use_planner:
                    try:
                        ...
                        events.append({"event": "planner", "selected": list(planner.task_keys), "rationale": planner.rationale})
                    except (RuntimeError, ValueError) as exc:
                        planner = PlannerDecision(tuple(t.key for t in candidates[:limit]), "deterministic fallback")
