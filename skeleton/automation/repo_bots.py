...
def run_tests(commands: list[str]) -> None:
    for command in commands[:3]:
        if not isinstance(command, str) or not command.strip():
            continue
        if any(x in command for x in ("&&", ";", "|", ">", "<", "`", "$", "sudo", "curl", "wget", "rm ")):
            raise RuntimeError("unsafe test command rejected")
        subprocess.run(command.split(), check=True, timeout=300)
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"status": "branch-created", "branch": branch, "summary": plan.get("summary", "")}, indent=2))
