...
def write_plan(plan: dict, *, root: Path | None = None) -> None:
    base = Path(".") if root is None else root
    for item in plan["files"]:
        target = base.joinpath(*PurePosixPath(item["path"]).parts)
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(item["content"], encoding="utf-8", newline="")


def run_tests(commands: object, plan: dict) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="skeleton-repo-bot-test-") as raw_tmp:
...
def main() -> int:
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", f"bot({args.mode}): bounded repository maintenance"], check=True)
