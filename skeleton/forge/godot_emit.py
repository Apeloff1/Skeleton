...
def emit_godot(
    ...
    *,
    ...
    files["scripts/autoloads/game_state.gd"] = (
        ...
        "func add_core(id: String) -> void:\n"
        "	if id not in data_cores_held:\n"
        "		data_cores_held.append(id)\n\n"
        "func can_enter(rid: String) -> bool:\n"
        "	if extract_late and rid == extract_room and data_cores_held.size() < 1:\n"
        "		return false\n"
        "	return true\n\n"
    ...
    files["scripts/world/door.gd"] = (
        ...
        "		if not GameState.can_enter(dest_room):\n"
        "			return\n"
...
def _world_map_gd(plan: Dict[str, Any], graph: Optional[Dict[str, Any]] = None) -> str:
    ...
    return (
        ...
        "		for r in rooms:\n"
        "			if r.kind == \"extract\":\n"
        "				GameState.extract_room = r.id\n"
        "		EventBus.room_entered.emit(rooms[0].id)\n"
        "	if EventBus.room_entered.is_connected(_on_room) == false:\n"
        "		EventBus.room_entered.connect(_on_room)\n\n"
        "func _on_room(rid: String) -> void:\n"
        "	for r in rooms:\n"
        "		if r.id == rid and (r.kind == \"loot\" or r.kind == \"combat\"):\n"
        "			GameState.add_core(rid)\n"
    )
