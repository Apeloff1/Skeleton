...
class JeevesSessionRuntime:
    ...
    _selected_policy_decision_id: str | None = None
    ...
    @property
    def selected_policy_decision_id(self) -> str | None:
        """Return the lifecycle-bound ledger identity for the selected policy."""
        return self._selected_policy_decision_id
    ...
    def begin(
        ...
        *,
        ...
        orient_record = self._record_decision(
            ...
            state={"skill": primary or objective, "mastery": self.knowledge_state.mastery(primary or objective), "pipeline_digest": pipeline.digest, "provenance_digest": control.provenance_digest},
            policy={"confidence": arbitration.confidence, "counterfactual_margin": control.policy_competition.margin if control.policy_competition else 0.0, "calibrated_reliability": control.policy_calibrator.snapshot(), "rejected_policies": rejected_policies, "executed_policy": selected_policy, "pipeline_digest": pipeline.digest, "provenance_digest": control.provenance_digest},
        )
        self._selected_policy_decision_id = orient_record.decision_id
        if control.policy_competition:
            for candidate in control.policy_competition.rejected:
                self._record_decision(
                    decision_id=f"{session_id}:alternative:{candidate.action.value}",
                    action=candidate.action.value,
                    rationale=candidate.rationale + ("counterfactual alternative", "not executed"),
                    state={"skill": primary or objective, "pipeline_digest": pipeline.digest, "provenance_digest": control.provenance_digest},
                    policy={"selected": selected_policy, "counterfactual": True, "calibrated_reliability": control.policy_calibrator.snapshot(), "pipeline_digest": pipeline.digest, "provenance_digest": control.provenance_digest},
        ...
        return RuntimePlan(
            ...
            self.phase,
            pipeline_kind,
            tuple(s.stage.value for s in pipeline.steps),
            ...
            transitions,
            selected_policy,
            control.policy_competition.margin if control.policy_competition else 0.0,
            rejected_policies,
            pipeline.digest,
            control.provenance_digest,
        )
    ...
    def record_outcome(self, student: StudentProfile, outcome: SessionOutcome) -> OutcomeResult:
        ...
        outcome_id = f"{self.session_id}:outcome:{self._next_decision_sequence()}"
        ...
        self._record_decision(
            ...
            evidence=(outcome_id,),
            state={"skill": outcome.skill_id, "score": outcome.score, "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
            policy={"executed_policy": policy or "unknown", "calibration": self.control.policy_calibrator.snapshot(), "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
    ...
    def transition(self, target: SessionPhase, *, rationale: str, evidence: Sequence[EvidenceGate] = ()) -> SessionTransition:
        ...
        decision_id = f"{self.session_id}:transition:{self._next_decision_sequence()}"
        self._record_decision(
            decision_id=decision_id,
            action=f"{kind.value}:{target.value}",
            rationale=(rationale,),
            state={"source": source.value, "target": target.value, "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
            policy={"gate_count": len(gate_set), "selected_policy": self._selected_policy or "unknown", "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
        )
    ...
    def record_evidence(
        ...
        **payload: str,
        ...
        if claim:
            ...
            evidence_id = f"{self.session_id}:evidence:{sequence}"
            strength = 1.0 if score is None else max(0.0, min(1.0, score))
            update = self.epistemics.observe(EpistemicEvidence(evidence_id=evidence_id, claim=claim, polarity=polarity, strength=strength, source_id=source_id, step=sequence))
            ...
            self._record_decision(
                ...
                action=f"observe:{claim}",
                rationale=update.rationale,
                evidence=(evidence_id,),
                state={"confidence": update.belief.confidence, "contradiction": update.contradiction, "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
                policy={"polarity": polarity.value, "selected_policy": self._selected_policy or "unknown", "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
            )
    ...
    def recover(self, *, reason: str) -> SessionTransition:
        ...
        decision_id = f"{self.session_id}:recover:{self._next_decision_sequence()}"
        self._record_decision(
            decision_id=decision_id,
            action=TransitionKind.REPAIR.value,
            rationale=(reason,),
            state={"source": source.value, "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
            policy={"selected_policy": self._selected_policy or "unknown", "pipeline_digest": self.pipeline_contract_digest, "provenance_digest": self.provenance_digest},
        )
    ...
    def _prepare_session(self, session_id: str) -> None:
        ...
        self._selected_policy_decision_id = None
    ...
    def _emit(self, phase: SessionPhase, event: str, payload: dict[str, str]) -> None:
        ...
        self.events.append(SessionEvent(self._event_sequence, phase, event, tuple(sorted(payload.items()))))
