"""Import-free audit of create_app-mounted sidecar routers versus API_ROUTES.

F-19 covers ``skeleton.api.routes``; F-27 covers GameForge/command sidecars.
``create_app`` still mounts the swarm family and cockpit. Those handlers are
absent from API_ROUTES. This snapshot reports that without importing the API
package or adding sidecar-only rows to the main-router charter.
"""
...
from .audit_parse import architecture_api_routes, mounted_router_handlers
from .capability_manifest import CAPABILITY_MANIFEST_VERSION


MOUNTED_ROUTE_AUDIT_KIND: Final = "mounted_route_audit"
...
def mounted_route_audit_snapshot() -> dict[str, object]:
    """Return a JSON-serializable mounted-sidecar documentation audit."""

    handlers = mounted_router_handlers()
...
def get_mounted_route_audit_row(route_id: str) -> dict[str, object]:
    """Return one mounted-route audit row by ``METHOD /path`` key."""
    ...
    for row in mounted_route_audit_snapshot()["routes"]:
        if row["key"] == key:
            return dict(row)
