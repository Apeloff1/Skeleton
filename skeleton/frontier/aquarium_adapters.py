...
def aquarium_state_payload(state: AquariumState) -> dict[str, object]:
    """Return the canonical JSON-compatible aquarium state representation."""
    ...
    return {
        "tanks": {
            tank_id: {
                ...
                "decorations": [
                    _decoration_payload(decoration) for decoration in tank.decorations
                ],
                "theme": tank.theme,
    ...
    for raw_key, raw_tank_value in raw_tanks.items():
        ...
        decorations = tuple(
            ...
            for item in _sequence(
                raw_tank.get("decorations", ()), "aquarium decorations"
            )
        )
        tanks[tank_id] = AquariumTankState(
            ...
            background=_optional_text(
                raw_tank.get("background"), "aquarium tank background"
            ),
        )

    owned_raw = _sequence(payload.get("owned_tanks"), "aquarium owned_tanks")
    owned_tanks = frozenset(
        _text(item, "aquarium owned tank id") for item in owned_raw
    )
    inventory_raw = _mapping(
        payload.get("owned_decorations", {}), "aquarium owned decorations"
    )
...
def aquarium_event_to_memory_item(event: DomainEvent) -> Mapping[str, Any]:
    """Validate and project one aquarium event into canonical memory metadata."""
    ...
    actual_digest = aquarium_state_digest(state)
