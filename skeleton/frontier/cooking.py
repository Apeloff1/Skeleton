...
catalog ownership remain source-owned. This module promotes only the portable
policy: recipe normalization, unlock checks, typed ingredient allocation, timed
kitchen slots, collection rewards and expiring cooking buffs.
"""
...
def _inventory(values: Mapping[str, int], field_name: str) -> dict[str, int]:
    ...
    for raw_item, raw_quantity in values.items():
        ...
        normalized[item] = _nonnegative_int(raw_quantity, f"{field_name} quantity for {item}")
    return normalized
...
def _effect_map(values: Mapping[str, Any], field_name: str) -> dict[str, float | int | str | bool]:
    if not isinstance(values, Mapping):
        raise TypeError(f"{field_name} must be a mapping")
    ...
    for raw_key, raw_value in values.items():
        ...
        if isinstance(raw_value, bool):
            ...
            normalized[key] = _normalized_text(raw_value, f"{field_name} value for {key}")
        else:
            raise TypeError(f"{field_name} value for {key} must be JSON-scalar compatible")
    return normalized
...
class CookingIngredient:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "quantity", _positive_int(self.quantity, "cooking ingredient quantity"))
        object.__setattr__(self, "min_size", _nonnegative_number(self.min_size, "cooking ingredient min_size"))
        if kind == "item":
            if self.item_id is None:
                raise ValueError("item cooking ingredient requires item_id")
            object.__setattr__(self, "item_id", _normalized_text(self.item_id, "cooking ingredient item_id"))
            if self.fish_ids:
...
class FishInventoryItem:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "species", _normalized_text(self.species, "fish inventory species"))
        object.__setattr__(self, "size", _nonnegative_number(self.size, "fish inventory size"))
...
class CookingRecipe:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "name", _normalized_text(self.name, "cooking recipe name"))
        object.__setattr__(self, "category", _normalized_text(self.category, "cooking recipe category").lower())
        object.__setattr__(self, "difficulty", _positive_int(self.difficulty, "cooking recipe difficulty"))
        ...
        for ingredient in self.ingredients:
            if not isinstance(ingredient, CookingIngredient):
                raise TypeError("cooking recipe ingredients must be CookingIngredient values")
        object.__setattr__(self, "cooking_time_seconds", _positive_int(self.cooking_time_seconds, "cooking recipe cooking_time_seconds"))
        object.__setattr__(self, "required_station", _normalized_text(self.required_station, "cooking recipe required_station"))
        object.__setattr__(self, "unlock_level", _positive_int(self.unlock_level, "cooking recipe unlock_level"))
        object.__setattr__(self, "rewards", _integer_map(self.rewards, "cooking rewards"))
        object.__setattr__(self, "stats_boost", _effect_map(self.stats_boost, "cooking stats_boost"))
...
class CookingJob:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "recipe_id", _normalized_text(self.recipe_id, "cooking job recipe_id"))
        object.__setattr__(self, "recipe_name", _normalized_text(self.recipe_name, "cooking job recipe_name"))
...
class ActiveCookingBuff:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "recipe_id", _normalized_text(self.recipe_id, "cooking buff recipe_id"))
        object.__setattr__(self, "name", _normalized_text(self.name, "cooking buff name"))
        object.__setattr__(self, "effects", _effect_map(self.effects, "cooking buff effects"))
        object.__setattr__(self, "expires_at", _aware(self.expires_at, "cooking buff expires_at"))
...
class KitchenState:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "ingredients", _inventory(self.ingredients, "kitchen ingredients"))
        ...
        object.__setattr__(self, "cooking_level", _positive_int(self.cooking_level, "kitchen cooking_level"))
        object.__setattr__(self, "cooking_xp", _nonnegative_int(self.cooking_xp, "kitchen cooking_xp"))
        object.__setattr__(self, "dishes_cooked", _nonnegative_int(self.dishes_cooked, "kitchen dishes_cooked"))
        for buff in self.active_buffs:
            if not isinstance(buff, ActiveCookingBuff):
                raise TypeError("kitchen active_buffs must contain ActiveCookingBuff values")
...
def recipe_from_record(record: Mapping[str, Any]) -> CookingRecipe:
    ...
    for raw in raw_ingredients:
        ...
        if kind == "item":
            ingredients.append(CookingIngredient(kind="item", item_id=_normalized_text(raw.get("item_id"), "cooking ingredient item_id"), quantity=_positive_int(raw.get("quantity"), "cooking ingredient quantity")))
        elif kind == "fish":
            raw_fish_ids = raw.get("fish_ids")
            if not isinstance(raw_fish_ids, Sequence) or isinstance(raw_fish_ids, (str, bytes)):
                raise TypeError("cooking fish_ids must be a sequence")
            ingredients.append(CookingIngredient(kind="fish", fish_ids=tuple(_normalized_text(value, "cooking ingredient fish_id") for value in raw_fish_ids), quantity=_positive_int(raw.get("quantity"), "cooking ingredient quantity"), min_size=_nonnegative_number(raw.get("min_size", 0), "cooking ingredient min_size")))
        else:
    ...
    consumed = {"id", "name", "category", "difficulty", "description", "ingredients", "cooking_time_seconds", "required_station", "unlock_level", "rewards", "stats_boost", "icon"}
    return CookingRecipe(
        ...
        difficulty=_positive_int(record.get("difficulty"), "cooking recipe difficulty"),
        description=str(record.get("description") or "").strip(),
        ingredients=tuple(ingredients),
        cooking_time_seconds=_positive_int(record.get("cooking_time_seconds"), "cooking recipe cooking_time_seconds"),
        required_station=_normalized_text(record.get("required_station"), "cooking recipe required_station"),
        unlock_level=_positive_int(record.get("unlock_level", 1), "cooking recipe unlock_level"),
        rewards={_normalized_text(key, "cooking reward key"): _nonnegative_int(value, f"cooking reward {key}") for key, value in raw_rewards.items()},
...
def recipe_is_unlocked(recipe: CookingRecipe, *, user_level: int, unlocked_recipes: Sequence[str] | frozenset[str] = ()) -> bool:
    level = _positive_int(user_level, "cooking user_level")
    normalized = {_normalized_text(value, "unlocked cooking recipe id") for value in unlocked_recipes}
    return recipe.id in normalized or level >= recipe.unlock_level


def ingredient_purchase_cost(cost: Mapping[str, int], quantity: int) -> Mapping[str, int]:
...
def refresh_kitchen(state: KitchenState, *, now: datetime) -> KitchenState:
    ...
    for slot in state.cooking_slots:
        if slot is not None and slot.status == "cooking" and current >= slot.complete_at:
...
def _allocate_fish(ingredients: Sequence[CookingIngredient], fish_inventory: Sequence[FishInventoryItem]) -> tuple[tuple[str, ...], tuple[FishInventoryItem, ...]]:
    ...
    for ingredient in ingredients:
        ...
        if len(selected) != ingredient.quantity:
            ...
            raise ValueError(f"not enough eligible fish for cooking requirement ({accepted}); need {ingredient.quantity}")
...
def start_cooking(state: KitchenState, recipe: CookingRecipe, *, user_level: int, fish_inventory: Sequence[FishInventoryItem], now: datetime) -> CookingStartPlan:
    current = _aware(now, "cooking now")
    state = refresh_kitchen(state, now=current)
    if not recipe_is_unlocked(recipe, user_level=user_level, unlocked_recipes=state.unlocked_recipes):
        raise ValueError(f"cooking recipe {recipe.id} is locked")
    try:
        slot_index = next(index for index, slot in enumerate(state.cooking_slots) if slot is None)
    ...
    for ingredient in recipe.ingredients:
        ...
        consumed_items[ingredient.item_id] = consumed_items.get(ingredient.item_id, 0) + ingredient.quantity
    consumed_fish, remaining_fish = _allocate_fish(recipe.ingredients, fish_inventory)
    job = CookingJob(recipe_id=recipe.id, recipe_name=recipe.name, started_at=current, complete_at=current + timedelta(seconds=recipe.cooking_time_seconds), status="cooking")
    slots = list(state.cooking_slots)
    slots[slot_index] = job
    next_state = replace(state, cooking_slots=tuple(slots), ingredients=remaining_items)
    return CookingStartPlan(kitchen=next_state, remaining_ingredients=remaining_items, remaining_fish=remaining_fish, consumed_ingredients=consumed_items, consumed_fish_ids=consumed_fish, slot=slot_index, job=job)


def collect_dish(state: KitchenState, recipe: CookingRecipe, *, slot: int, now: datetime) -> CookingCollectPlan:
    current = _aware(now, "cooking now")
    state = refresh_kitchen(state, now=current)
    ...
    if recipe.stats_boost:
        ...
        effects = {key: value for key, value in recipe.stats_boost.items() if key != "duration_minutes"}
        if effects:
            buff = ActiveCookingBuff(recipe_id=recipe.id, name=recipe.name, effects=effects, expires_at=current + timedelta(minutes=duration_minutes))
            next_buffs = (*next_buffs, buff)
    slots = list(state.cooking_slots)
    slots[slot] = None
    xp_earned = _nonnegative_int(recipe.rewards.get("xp", 0), "cooking xp reward")
    next_state = replace(state, cooking_slots=tuple(slots), cooking_xp=state.cooking_xp + xp_earned, dishes_cooked=state.dishes_cooked + 1, active_buffs=tuple(next_buffs))
    return CookingCollectPlan(kitchen=next_state, recipe_id=recipe.id, rewards=dict(recipe.rewards), buff=buff, xp_earned=xp_earned)


def active_buffs(state: KitchenState, *, now: datetime) -> tuple[ActiveCookingBuff, ...]:
