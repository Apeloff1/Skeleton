...
class TargetResolver:
    ...
    def _open_prs(
        ...
        *,
        ...
        pages = max(1, min(self.policy.limits.max_pages, (limit + 99) // 100))
        items, complete = self.transport.paged_list(
    ...
    def resolve(self, identity: RunIdentity) -> TargetSet:
        ...
        if identity.explicit_pr is not None:
            self._fetch_pr(identity.repository, identity.explicit_pr)
            return TargetSet(
                repository=identity.repository,
                targets=(
                    _target(
