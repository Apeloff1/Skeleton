...
class MutableBudget:
    ...
    def consume_graphql(self, count: int = 1) -> RunBudget:
        if count < 0:
            raise ValueError("GraphQL count must be non-negative")
...
def budget_allows_mutation(
    ...
    *,
    ...
    if observation.rate_remaining is not None:
        if observation.rate_remaining < policy.limits.minimum_rate_remaining:
