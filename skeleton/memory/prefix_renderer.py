"""Prefix renderer — KV-cache-friendly prompt prefixes for the memory plane.

Ported from ``backend/services/cag.py`` (Track B2 of the build plan): the
expensive part of an LLM call is re-processing the same long static prefix
on every request. This module renders the static context (persona, system
laws, learning stages) ONCE into a deterministic, content-hashed prefix
block; identical hashes across requests mean the provider's KV-cache hits
and the prefix is billed at the cached (~10%) rate.
...
Pure domain: no I/O, no framework imports. The backend service module keeps
serving its own copy until the adapter cutover (B4) — this is the canonical
home going forward.
"""
...
# ── Single-sourced static prefix text (canonical Tutolage/Jeeves content) ──
...
class PrefixRegistry:
    """Process-wide prefix registry, keyed by prefix key.

    Callers compare ``.sha`` to detect drift; the memory warmer
    (:mod:`skeleton.memory.warmer`) owns refresh policy.
    """
    ...
    def get(self, key: str) -> Optional[CAGPrefix]:
...
class PrefixRenderer:
    """Builds and caches the canonical Jeeves tutoring prefix.

    Rebuilds only when the content hash drifts — the stable bytes are what
    keep the provider KV-cache warm.
    """

    JEEVES_SYSTEM_KEY = "jeeves:system"

    def __init__(self, registry: Optional[PrefixRegistry] = None) -> None:
        self.registry = registry or PrefixRegistry()

    def jeeves_system_prefix(self) -> CAGPrefix:
        segments = [
    ...
    def compose_prompt(
        ...
        Retrieved snippets sit BETWEEN prefix and tail: they vary per request
        but are short, so they only invalidate the tail of the KV cache, not
        the expensive prefix.
        """
        parts = [prefix.text]
        if retrieved:
