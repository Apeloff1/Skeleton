...
def _broker(runtime: SwarmRuntime = Depends(_runtime)) -> SwarmBroker:
    ...
    if broker is None or broker.runtime is not runtime:
        ...
        broker = SwarmBroker(runtime, supervisor=supervisor)
        state.swarm_broker = broker
        if getattr(state, "swarm_supervisor", None) is None:
            state.swarm_supervisor = broker.supervisor
    return broker
...
def complete(worker_id: str, task_id: str, broker: SwarmBroker = Depends(_broker)) -> dict[str, Any]:
    try:
        task = broker.record_success(worker_id, task_id)
    except LeaseError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    return {"task_id": task.id, "state": task.state.value, "worker_id": worker_id}
...
def fail(worker_id: str, task_id: str, body: BrokerFailure, broker: SwarmBroker = Depends(_broker)) -> dict[str, Any]:
    try:
        task = broker.record_failure(worker_id, task_id, body.error)
    except LeaseError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    return {
        "task_id": task.id,
