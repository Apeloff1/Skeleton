...
def frontier_semantic_lenses() -> tuple[SemanticLensSpec, ...]:
    ...
    return (
        ...
        _lens("point_of_audition", LensFamily.FILM, SemanticRole.PERSPECTIVE, 1980,
              "Treat soundtrack access as viewpoint-conditioned: what is audible, attenuated, masked, or absent may depend on the listener position rather than world state.",
              ("hears", "muffled", "audible", "soundproof", "distance", "headphones", "listening", "audition"),
              ("Whose auditory position generates this sound field?", "Which sounds could exist in the world while remaining unavailable to this listener?"),
              "Changing listener position or sensory access predicts a systematic change in the audible field without requiring a change in the underlying source.",
              "Treating viewpoint-conditioned audibility as proof that an unheard sound did not occur.", minimum=2, sequential=True, rare=True),
        _lens("embodied_sound_perspective", LensFamily.FILM, SemanticRole.PERSPECTIVE, 2026,
              "Model sound that represents a character's bodily or perceptual auditory state rather than only spatial point of audition.",
              ("tinnitus", "hearing", "deaf", "cochlear", "subjective sound", "muffled", "distorted audio", "inside head"),
              ("Is the soundtrack representing external acoustics, listener location, or embodied perception?",),
              "Independent external observations can remain stable while the rendered soundtrack changes with the character's sensory state.",
              "Medicalizing or psychologizing stylized sound without enough contextual evidence.", minimum=2, sequential=True, rare=True),
        ...
        _lens("narrative_frequency", LensFamily.NARRATIVE, SemanticRole.STRUCTURE, 1972,
              "Distinguish how often an event occurs in the story from how often it is narrated: singulative, iterative, and repeating discourse are different evidence structures.",
              ("every time", "usually", "again", "repeats", "once", "whenever", "used to", "each day", "told again"),
              ("Did the event happen repeatedly, was one event narrated repeatedly, or is one statement summarizing many events?", "Which count belongs to story events and which belongs to discourse mentions?"),
              "Separating event frequency from telling frequency prevents repeated mentions from being mistaken for independent repeated events.",
              "Counting repeated narration as independent event evidence or collapsing an iterative summary into one event.", minimum=2, sequential=True, rare=True),
        _lens("discourse_duration_ratio", LensFamily.NARRATIVE, SemanticRole.STRUCTURE, 1972,
              "Compare story-time span with discourse attention so pauses, scenes, summaries and ellipses are treated as different temporal compression regimes.",
              ("years passed", "moments later", "in detail", "summary", "pause", "scene", "ellipsis", "meanwhile"),
              ("How much event time is represented by how much discourse attention?",),
              "Abrupt changes in discourse/story-time ratio predict shifts in salience or omitted-event uncertainty, not necessarily changes in event importance.",
              "Equating narrative duration with objective duration or assuming long description proves greater factual importance.", minimum=2, sequential=True, rare=True),
        ...
        _lens("ludic_diegetic_interaction_levels", LensFamily.GAME, SemanticRole.META, 2017,
              "Separate game-maker-to-player communication at the ludic level from character-to-character communication at the diegetic level.",
              ("dialogue choice", "option", "player", "character says", "npc", "script", "choice wheel", "roleplay"),
              ("Is this choice communicating an in-world intention, a game-system option, or both?", "Does friction arise because the player's inferred option differs from the character's realized act?"),
              "Mismatches between ludic option cues and diegetic realization predict surprise, agency loss, or systematic choice errors.",
              "Collapsing player intent, interface semantics, and character intent into one state.", minimum=2, pairwise=True, rare=True),
        _lens("affordance_feedback_triad", LensFamily.GAME, SemanticRole.CAUSAL_HINT, 2000,
              "Keep actual action possibility, perceived affordance, and system feedback as three separate variables.",
              ("can interact", "looks clickable", "button", "feedback", "prompt", "highlight", "door", "usable", "affordance"),
              ("What action is mechanically possible?", "What action does the representation suggest?", "What feedback updates the player's belief after acting?"),
              "User behavior is better predicted when real affordance, perceived affordance, and feedback are modeled separately.",
              "Inferring mechanics from appearance alone or treating feedback as the action's causal effect.", minimum=2, sequential=True, rare=True),
        _lens("dialogue_option_execution_gap", LensFamily.GAME, SemanticRole.ADVERSARIAL_READING, 2025,
              "Compare the pragmatic action implied by a dialogue option with the utterance and consequence the game actually executes.",
              ("dialogue option", "choice", "paraphrase", "said", "meant", "wrong answer", "forced answer", "but thou must"),
              ("What pragmatic act did the player reasonably select?", "What act did the character actually perform?", "Did downstream state respect that distinction?"),
              "Persistent cue-to-execution mismatch predicts frustration and lets future dialogue choices be modeled as partially deceptive interface signals.",
              "Assuming every paraphrase difference is an agency failure or that all constrained dialogue is defective.", minimum=2, sequential=True, rare=True),
        _lens("substrate_simulation_experience_stack", LensFamily.GAME, SemanticRole.META, 2005,
              "Separate the generative rule substrate, the simulated world trajectory, and the player's experiential observation channel.",
              ("rules", "simulation", "camera", "audio", "experience", "world state", "rendered", "substrate"),
              ("Which fact belongs to rules, simulated state, or player-visible experience?", "Could the simulated state differ while the experiential channel looks identical?"),
              "Layer-specific interventions predict different effects: rule changes alter reachable worlds, state changes alter one trajectory, and presentation changes alter observations without necessarily changing simulation state.",
              "Treating rendered experience as the complete simulation state or treating a rule as if it were an observed event.", minimum=2, sequential=True, rare=True),
...
def default_interaction_rules() -> tuple[LensInteractionRule, ...]:
    return (
        LensInteractionRule(
            "point_of_audition", "subjective_camera", LensInteractionKind.REINFORCES,
            "Auditory and visual viewpoint can independently condition access to the same world without changing the world state.",
            "Do visual and auditory access identify the same observer, and which hidden variables remain outside both channels?",
            "Aligned multimodal viewpoint restrictions predict consistent observer-specific omissions; disagreement suggests channel-specific mediation.",
            tangent_axis_hint="cinematic",
        ),
        LensInteractionRule(
            "narrative_frequency", "motif_recurrence", LensInteractionKind.CONDITIONS,
            "Repeated mention and repeated event are not equivalent; motif recurrence becomes evidential only after discourse repetition is separated from event recurrence.",
            "Is the motif recurring in world events, in the telling of one event, or in an iterative summary of many events?",
            "Event-level recurrence should predict future occurrences more strongly than repeated retelling of one past occurrence.",
            tangent_axis_hint="temporal",
        ),
        LensInteractionRule(
            "ludic_diegetic_interaction_levels", "diegetic_interface", LensInteractionKind.REINFORCES,
            "Both separate player-facing information from in-world agency, but one concerns communicative level while the other concerns information access.",
            "Did the player know/select something that the character neither knew nor enacted?",
            "A persistent level mismatch predicts choice errors even when the interface exposes accurate factual information.",
            tangent_axis_hint="ludic",
        ),
        LensInteractionRule(
            "affordance_feedback_triad", "false_affordance", LensInteractionKind.REINFORCES,
            "False affordance is one failure case of a broader actual/perceived/feedback decomposition.",
            "Was the mismatch in perceived possibility, actual transition availability, or misleading feedback after the action?",
            "Separating the three variables should localize repeated interaction failures to presentation, mechanics, or feedback.",
            tangent_axis_hint="ludic",
        ),
        LensInteractionRule(
            "dialogue_option_execution_gap", "ludic_diegetic_interaction_levels", LensInteractionKind.REINFORCES,
            "Dialogue-option mismatch is a concrete place where player-system communication and character-level action can diverge.",
            "Does the option accurately encode the pragmatic act that the character later performs?",
            "Option classes with poor cue-to-execution fidelity should show higher prediction error for player expectations.",
            tangent_axis_hint="ludic",
        ),
        LensInteractionRule(
            "substrate_simulation_experience_stack", "possibility_space", LensInteractionKind.CONDITIONS,
            "Possibility space belongs primarily to the rule substrate, while any one playthrough and viewpoint reveal only a subset of reachable states.",
            "Is a missing possibility actually forbidden by rules, merely absent from this trajectory, or hidden from the player's observation channel?",
            "Interventions at the correct layer should discriminate formal impossibility from unvisited or unobserved states.",
