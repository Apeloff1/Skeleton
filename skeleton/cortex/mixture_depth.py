...
class MixtureOfDepths:
    ...
    threshold:
        ...
        force tokens to full depth and are useful for calibration.
    min_depth:
    ...
    def __init__(
        ...
        *,
        threshold: float | Sequence[float] = 0.015,
        min_depth: int = 1,
        enabled: bool = True,
    ) -> None:
        ...
        if isinstance(threshold, (int, float)):
            ...
            thresholds = tuple(float(value) for value in threshold)
        if not thresholds:
            raise ValueError("threshold schedule cannot be empty")
        if any(math.isnan(value) for value in thresholds):
            raise ValueError("threshold schedule cannot contain NaN")
        depth = int(min_depth)
        if depth < 1 or depth > model.n_layers:
