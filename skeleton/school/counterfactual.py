...
def compete(candidates: Sequence[PolicyCandidate]) -> CounterfactualResult:
    ...
    rationale = selected.rationale + (f"selected by deterministic score={selected.score:.4f}", f"selection margin={margin:.4f}")
...
def default_candidates(*, mastery: float, contradiction: float, energy: float, transfer_ready: bool) -> tuple[PolicyCandidate, ...]:
    mastery=max(0.0,min(1.0,mastery)); contradiction=max(0.0,min(1.0,contradiction)); energy=max(0.0,min(1.0,energy))
    return (
