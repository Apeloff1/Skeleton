...
def _cmd_plan(rest: List[str]) -> int:
    """BuildPlan without cortex.live — stay off #25/#50 Jeeves restore path."""
    from skeleton.forge.eras import compile_era, list_eras
    from skeleton.jeeves.builder import BuilderBrain
    vision = " ".join(rest).strip().lower()
    eras = list(list_eras())
    era = next((e for e in eras if e.replace("_", " ") in vision or e in vision), None)
    if era is None:
        era = "soulslike" if "soulslike" in vision else (eras[0] if eras else "extraction_now")
    pack = compile_era(era)
    plan = BuilderBrain().plan(pack)
    out = plan.to_dict() if hasattr(plan, "to_dict") else {
        "era": getattr(plan, "era", era),
        "room_bias": getattr(plan, "room_bias", None),
