...
class PipelineAgent:
    ...
    async def run(self, task: str, context: Mapping[str, Any] | None = None) -> Any:
        ...
        process = await asyncio.create_subprocess_exec(
            sys.executable, "-m", "skeleton.services.frontier_worker", self.name,
...
def create_frontier_runtime(*, state_root: str | Path = ".skeleton",
                            execution_policy: ExecutionPolicy | None = None) -> AgentRuntime:
    runtime = AgentRuntime(policy=CapabilityPolicy.from_names(OPERATIONS.values()),
                           execution_policy=execution_policy, events=EventBus())
