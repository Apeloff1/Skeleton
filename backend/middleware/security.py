"""
security.py — fail-closed security middleware for the FastAPI app.

Provides:
  1. RateLimitMiddleware       — bounded per-IP+route token bucket
  2. AuditMiddleware           — bounded request audit ring
  3. SizeLimitMiddleware       — raw-ASGI request framing/body cap
...
def _matches_route_boundary(path: str, route: str) -> bool:
    """Return true only for an exact route or a real child route."""
...
def _parse_trusted_proxy_networks(
    ...
    for entry in raw.split(","):
        ...
        try:
            ...
            # Invalid config never broadens the trust boundary.
...
def _client_ip(
    ...
    The direct peer is authoritative unless explicitly configured as trusted.
    A trusted proxy chain is walked right-to-left. Duplicate, empty, malformed,
    or entirely trusted chains fail closed to the direct peer.
    """
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    Unknown identities and new identities at capacity fail closed. Active bucket
    state is never evicted merely to admit churned attacker identities.
    """
            ...
            "top": [
                {
                    "ip": k[0],
                    "route": k[1],
                    "tokens_remaining": round(v.tokens, 2),
                }
                for k, v in sorted(
                    cls._buckets.items(), key=lambda kv: kv[1].tokens
                )[:20]
            ],
        }
...
class AuditMiddleware(BaseHTTPMiddleware):
    ...
    def _declared_size(request: Request) -> int | None:
        ...
        # Keep audit metadata bounded even before body framing rejects the request.
...
# ─────────────────────────────────────────────────────────────────
# Body-size/framing cap — raw ASGI so streamed bodies cannot bypass it
# ─────────────────────────────────────────────────────────────────
class SizeLimitMiddleware:
    ...
    def __init__(
        ...
        configured_mb = (
            ...
            else int(os.environ.get("CODEDOCK_MAX_BODY_MB", "25"))
        )
    ...
    async def __call__(self, scope, receive, send):
        ...
        if content_lengths:
            ...
            if (
                not raw_declared.isascii()
                or not raw_declared.isdigit()
