"""Regression tests for repository-wide tar extraction safety coverage."""
...
GATE_PATH = REPO_ROOT / "scripts" / "check_repository_archive_extraction_safety.py"
SPEC = importlib.util.spec_from_file_location("repository_archive_extraction_safety", GATE_PATH)
...
def test_repository_archive_scope_covers_core_and_tooling() -> None:
    files = list(GATE.production_python_files())
...
def test_repository_wrapper_reuses_archive_violation_engine(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path,
        "import tarfile\nwith tarfile.open('x.tar') as archive:\n"
...
def test_repository_wrapper_allows_data_filter(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path,
        "import tarfile\nwith tarfile.open('x.tar') as archive:\n"
...
def test_repository_archive_fails_closed_when_required_root_missing(
    tmp_path: Path,
...
def test_repository_archive_requires_every_root_nonempty(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_repository_archive_accepts_clean_nonempty_roots(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_repository_archive_rejects_symlinked_required_root(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
