...
def test_pr_hygiene_runs_only_when_diff_classification_can_change() -> None:
    text = WORKFLOW.read_text(encoding="utf-8")

    assert "types: [opened, reopened, synchronize]" in text
    for noisy in ("edited", "ready_for_review", "converted_to_draft", "closed"):
        assert noisy not in text.split("concurrency:", 1)[0]
    assert "github.event.action != 'closed'" not in text
