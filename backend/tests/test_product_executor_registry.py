...
def test_registry_requires_exact_capability_action_binding(tmp_path):
    ...
    binding = registry.register(
        "studio", "build.submit", lambda operation, payload: True,
...
def test_registry_snapshot_is_deterministic_and_introspectable():
    registry = ProductExecutorRegistry()
    registry.register("world-forge", "world.create", lambda operation, payload: True,
                      name="world", replay_safe=True)
    registry.register("studio", "project.create", lambda operation, payload: True,
                      name="project", effect_class="state")
...
def test_default_control_plane_reports_native_coverage(tmp_path):
    plane = ProductControlPlane(tmp_path)
    ...
    assert coverage["bound_actions"] == 4
    assert coverage["coverage_pct"] > 0
    assert {item["action"] for item in coverage["missing"]} >= {"build.submit", "jeeves.reason"}
