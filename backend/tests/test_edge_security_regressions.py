...
def test_rate_limiter_prunes_idle_state_and_stays_bounded(monkeypatch) -> None:
    ...
    now = 1000.0
    RateLimitMiddleware._buckets[("198.51.100.1", "/api/run")] = security._Bucket(
        0, now - 31
    )
    RateLimitMiddleware._buckets[("198.51.100.2", "/api/run")] = security._Bucket(
        0, now
    )
...
def test_safe_relative_path_rejects_nul(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        security.safe_relative_path(tmp_path, "ok\x00evil")
