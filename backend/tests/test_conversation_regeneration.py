...
def _now():
    return datetime(2026, 9, 24, 0, 5, tzinfo=timezone.utc)
...
def _assistant(thread, user) -> ConversationMessage:
    return ConversationMessage(
        message_id=str(uuid4()),
        thread_id=thread.thread_id,
        branch_id=thread.active_branch_id,
...
@pytest.mark.asyncio
async def test_regeneration_preserves_causal_user_and_creates_fresh_branch():
    thread = _thread()
    user = _user(thread)
...
@pytest.mark.asyncio
async def test_regeneration_rejects_non_assistant_source():
    thread = _thread()
    prior = _user(thread)
    authority = object.__new__(MongoConversationAuthority)
