"""Regression tests for repository-wide dynamic-import safety coverage."""
...
GATE_PATH = REPO_ROOT / "scripts" / "check_repository_dynamic_import_safety.py"
SPEC = importlib.util.spec_from_file_location("repository_dynamic_import_safety", GATE_PATH)
...
def test_repository_dynamic_import_scope_covers_core_and_tooling() -> None:
...
def test_repository_wrapper_reuses_dynamic_import_violation_engine(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path,
...
def test_repository_wrapper_allows_literal_dynamic_import_target(tmp_path: Path) -> None:
    findings = _scan(
        tmp_path,
...
def test_repository_dynamic_import_fails_closed_when_required_root_missing(
    tmp_path: Path,
...
def test_repository_dynamic_import_requires_every_root_nonempty(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_repository_dynamic_import_accepts_clean_nonempty_roots(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
...
def test_repository_dynamic_import_rejects_symlinked_required_root(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
