"""Regression coverage for request ID validation, propagation, and log safety."""
...
def _run_rate_limited(
    request_ids: str | list[str] | None,
) -> tuple[str, str, str]:
    request = _request(request_ids)
    ...
    bucket, _retry = limiter._bucket_for("192.0.2.10")
...
def test_generated_ids_do_not_repeat_across_adversarial_sample() -> None:
    generated = {_assert_generated(None) for _ in range(256)}
...
def test_rate_limit_short_circuit_rejects_duplicate_request_ids() -> None:
    generated = _assert_generated_triplet(
        _run_rate_limited(["first", "second"])
    )
...
def test_rate_limit_log_cannot_be_line_injected_by_request_id(caplog) -> None:
    hostile = "attacker\nforged-log-line"
    caplog.set_level(logging.WARNING, logger="api.middleware")

    generated = _assert_generated_triplet(_run_rate_limited(hostile))

    matching = [record for record in caplog.records if record.name == "api.middleware"]
    assert matching
    assert hostile not in caplog.text
    assert generated in caplog.text
    assert all("\n" not in record.getMessage() for record in matching)
