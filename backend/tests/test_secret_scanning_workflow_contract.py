...
def test_push_runs_keep_per_generation_sha_scoped_evidence() -> None:
    text = _workflow_text()

    assert "group: secret-scanning-${{ github.event.pull_request.number || github.sha }}" in text
    assert "cancel-in-progress: ${{ github.event_name == 'pull_request' }}" in text
    assert "github.ref_name" not in text.split("permissions:", 1)[0]


def test_existing_push_range_binds_exact_before_and_current_commit() -> None:
    push_case = _push_case(_workflow_text())
