...
def test_rate_limiter_prunes_expired_state_and_stays_bounded():
...
def test_rate_limiter_hard_cap_rejects_concurrent_high_cardinality_creation():
    async def exercise() -> None:
        limiter = RateLimiterMiddleware(
...
def test_rate_limiter_preserves_active_bucket_when_capacity_is_full():
    async def exercise() -> None:
        limiter = RateLimiterMiddleware(
            _App(), per_minute=60, burst=2, max_buckets=2, bucket_ttl=300
        )
...
def test_recent_access_reorders_bucket_for_constant_time_expiry_pruning():
    async def exercise() -> None:
        limiter = RateLimiterMiddleware(
...
def test_saturation_churn_cannot_reset_an_existing_exhausted_bucket():
    async def exercise() -> None:
        limiter = RateLimiterMiddleware(
...
def test_expiry_reopens_capacity_without_active_eviction():
    async def exercise() -> None:
        limiter = RateLimiterMiddleware(
            _App(), per_minute=60, burst=1, max_buckets=2, bucket_ttl=30
        )
