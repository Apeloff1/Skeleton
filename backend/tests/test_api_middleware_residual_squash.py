"""Residual adversarial regressions after API middleware convergence."""
...
def test_forwarded_chain_at_hop_budget_still_resolves(monkeypatch) -> None:
    monkeypatch.setattr(
        api_middleware,
        "_TRUSTED_PROXY_NETWORKS",
        (ipaddress.ip_network("10.0.0.0/8"),),
    )
    chain = ",".join(["203.0.113.7", *("10.0.0.8" for _ in range(31))])
    request = _request(peer="10.0.0.10", xff=chain)
