...
def _pr(number: int, *, sha: str, head_ref: str = HEAD_REF, state: str = "closed") -> dict[str, object]:
    return {
        "number": number,
        "state": state,
        "head": {
...
def _history_path() -> str:
    return (
        f"/repos/{REPO}/pulls?state=all&head=Apeloff1%3Afeature%2Fcleanup"
        "&base=main&sort=updated&direction=desc&per_page=100"
    )
...
def test_resolve_closed_unmerged_pr_from_exact_branch_and_sha_history() -> None:
    ...
    api = ScriptedApi(
        {
            commit_path: (200, []),
            _history_path(): (200, [_pr(867, sha="older-head"), _pr(868, sha="closed-head")]),
        }
    )
...
def test_zero_commit_and_history_match_is_benign_noop() -> None:
    ...
    api = ScriptedApi({commit_path: (200, []), _history_path(): (200, [_pr(868, sha="other")])})
...
def test_commit_association_ambiguity_fails_without_history_fallback() -> None:
    ...
    api = ScriptedApi({commit_path: (200, [_pr(868, sha="shared-head"), _pr(869, sha="shared-head")])})
...
def test_branch_history_ambiguity_fails_closed() -> None:
    ...
    api = ScriptedApi(
        {
            commit_path: (200, []),
            _history_path(): (200, [_pr(868, sha=HEAD_SHA), _pr(869, sha=HEAD_SHA)]),
        }
    )
...
def test_history_api_failure_fails_closed() -> None:
    commit_path = f"/repos/{REPO}/commits/{HEAD_SHA}/pulls"
    api = ScriptedApi({commit_path: (200, []), _history_path(): (503, {})})
...
def test_main_skips_no_target_without_calling_privileged_drainer(monkeypatch: pytest.MonkeyPatch) -> None:
    commit_path = f"/repos/{REPO}/commits/{HEAD_SHA}/pulls"
    api = ScriptedApi({commit_path: (200, []), _history_path(): (200, [])})
...
def test_open_pr_defers_until_rest_head_matches_signal() -> None:
    ...
    assert not live_pr_head_converged(api, repo=REPO, pr_number=681, signal_head_sha="new-head")
...
def test_open_pr_allows_cleanup_after_rest_head_converges() -> None:
    ...
    assert live_pr_head_converged(api, repo=REPO, pr_number=681, signal_head_sha="new-head")
...
def test_unknown_pr_state_fails_closed() -> None:
    ...
    with pytest.raises(RuntimeError, match="unknown state"):
        live_pr_head_converged(api, repo=REPO, pr_number=681, signal_head_sha="new-head")
