...
def test_ai_pipeline_broad_failures_cannot_reach_http_error_detail() -> None:
    tree = ast.parse(AI_PIPELINE.read_text(encoding="utf-8"), filename=str(AI_PIPELINE))
    leaks: list[int] = []
    for handler in (node for node in ast.walk(tree) if isinstance(node, ast.ExceptHandler)):
        if not _is_broad_exception_handler(handler) or not handler.name:
            continue
        for statement in handler.body:
            for call in _http_exception_calls(statement):
                if _references_name(call, handler.name):
                    leaks.append(call.lineno)
    assert leaks == [], f"AI pipeline exposes caught exception data at lines {leaks}"
