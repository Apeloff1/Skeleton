...
def test_compose_keeps_provider_credentials_out_of_backend() -> None:
...
def test_manifest_declares_engine_provider_credential_ownership() -> None:
    manifest = json.loads(
        (REPO_ROOT / "skeleton" / "app" / "manifest.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert runtime["credential_owners"]["OPENAI_API_KEY"] == "skeleton"
    assert "GF_SEAL_SECRET" in runtime["required_env"]
    assert "EMERGENT_LLM_KEY" not in runtime["optional_env"]
    assert construction["provider_credential_owner"] == "skeleton"
    assert construction["backend_provider_mode"] == "engine-client-only"
    assert construction["engine_execution_owner"] == "skeleton"
