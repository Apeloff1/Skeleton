...
class SourceIndependenceAnalyzer:
    ...
    def _ancestry(self, source_id: str) -> tuple[set[str], set[str], SourceNode | None]:
        ...
        if node is None:
            return set(), set(), None
        seen: set[str] = set(); roots: set[str] = set(); stack = list(node.parent_ids)
        while stack:
            current = stack.pop()
            ...
            if parent is None:
                roots.add(current); continue
            if parent.parent_ids: stack.extend(parent.parent_ids)
            else: roots.add(parent.source_id)
        if not node.parent_ids: roots.add(node.source_id)
        ...
        for i, a in enumerate(ids):
            for b in ids[i + 1:]:
                if a in ancestry.get(b, set()) or b in ancestry.get(a, set()):
                    self._union(parent, a, b); continue
