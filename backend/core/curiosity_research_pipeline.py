...
Model ensemble output is hypothesis generation, not empirical verification.
Source adapters must explicitly bind evidence to exact claims, identify source
independence, evidence kind, quality and reproducibility. Unbound citations do not
promote claims. This blocks citation laundering and model-consensus hallucination.
"""
...
def _extract_json(value: str | dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        parsed = json.loads(text); return parsed if isinstance(parsed, dict) else {}
    except json.JSONDecodeError:
        match = re.search(r"\{.*\}", text, re.S)
        if not match: return {"summary": text[:8000]}
        try:
            parsed = json.loads(match.group(0)); return parsed if isinstance(parsed, dict) else {"summary": text[:8000]}
        except json.JSONDecodeError:
...
def _texts(value: Any, *, limit: int = 32) -> tuple[str, ...]:
    ...
    for item in value:
        text = " ".join(str(item).split()).strip(); key = text.casefold()
        if text and key not in seen: seen.add(key); out.append(text[:3000])
...
class EnsembleCuriosityResearcher:
    def __init__(self, completion: Completion, *, models: tuple[str, ...] = ("reasoning", "balanced", "creative"),
        ...
        if not models: raise ValueError("at least one research model is required")
        self.completion = completion; self.models = models; self.source_search = source_search
    ...
    def _prompt(inquiry: Inquiry, context: dict[str, Any]) -> str:
        return json.dumps({"subject": inquiry.subject, "questions": inquiry.questions,
            "known_verified_context": context.get("working_context", ()), "known_verified_claims": context.get("claims", ()),
            "known_unresolved": context.get("unresolved", ()),
            "task": "Generate falsifiable candidate claims, disagreements, gaps and adjacent concepts. Do not represent a claim as verified."}, ensure_ascii=False)

    async def _observe(self, model: str, inquiry: Inquiry, context: dict[str, Any]) -> ModelObservation:
        raw = await self.completion(model, self._prompt(inquiry, context), self._system_prompt()); data = _extract_json(raw)
        return ModelObservation(model=model, summary=" ".join(str(data.get("summary") or "").split())[:8000],
    ...
    def _source(raw: dict[str, Any]) -> ResearchSource | None:
        ...
        return ResearchSource(
            source=source[:1000], locator=str(raw.get("locator") or "")[:2000], excerpt=str(raw.get("excerpt") or "")[:6000],
            quality=quality, kind=str(raw.get("kind") or "unsourced"), independence_group=group[:500],
            supports_claims=_texts(raw.get("supports_claims"), limit=32), contradicts_claims=_texts(raw.get("contradicts_claims"), limit=32),
            ...
            verified_locator=bool(raw.get("verified_locator", False)),
        )

    async def __call__(self, inquiry: Inquiry, context: dict[str, Any]) -> dict[str, Any]:
        observations = await asyncio.gather(*(self._observe(model, inquiry, context) for model in self.models), return_exceptions=True)
        ...
        claim_votes: dict[str, tuple[str, int]] = {}; questions: list[str] = []; contradictions: list[str] = []; tags: list[str] = []; summaries: list[str] = []
        for obs in valid:
            if obs.summary: summaries.append(obs.summary)
            for claim in obs.claims:
                key = claim.casefold(); original, votes = claim_votes.get(key, (claim, 0)); claim_votes[key] = (original, votes + 1)
        ...
        contradictions.extend(f"MODEL PANEL DISAGREEMENT — candidate not independently established: {claim}" for claim in single_model[:16])
        ...
        if self.source_search is not None:
            for raw in await self.source_search(inquiry, inquiry.questions):
                ...
                if len(sources) >= self.max_sources: break

        # Exact normalized claim binding only. A source mentioning a topic is not
        # enough; the adapter must say which candidate claim it supports/refutes.
        ...
        for source in sources:
            if not source.verified_locator:
                continue
            common = {"source_id": source.source, "source": source.source, "locator": source.locator,
            ...
            for raw_claim in source.supports_claims:
                ...
                if claim is not None: claim_evidence[claim].append({**common, "supports": True})
            for raw_claim in source.contradicts_claims:
                claim = canonical_claims.get(raw_claim.casefold())
                if claim is not None: claim_evidence[claim].append({**common, "supports": False})
        ...
        return {
            ...
            # Panel metadata is retained for audit but cannot satisfy empirical gates.
            "panel": [asdict(obs) for obs in valid], "source_count": len(sources),
