...
class ProductOperationCoordinator:
    ...
    def _load_index(self) -> dict[str, dict[str, Any]]:
        ...
        if not isinstance(raw, dict):
            raise OperationRejected("operation idempotency index must be an object")
        return {str(key): dict(value) for key, value in raw.items() if isinstance(value, dict)}

    def _persist_index(self) -> None:
        temp = self._index_path.with_suffix(".tmp")
        with temp.open("w", encoding="utf-8") as handle:
            json.dump(self._idempotency, handle, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
            handle.flush()
    ...
    def admit(
        ...
        *,
        ...
        with self._lock:
            ...
            # Preflight before immutable staging so capacity exhaustion cannot
            # strand an otherwise-unreferenced artifact manifest.
    ...
    async def execute_one(self, seq: int, executor: Executor) -> ExecutionResult:
        ...
        try:
            ...
            self.audit.append(
                ...
                detail=json.dumps(
                    {"operation_id": operation.id, "outbox_seq": seq, "error": type(exc).__name__},
                    sort_keys=True,
                    separators=(",", ":"),
                ),
            )
        ...
        if not executed:
            audit = self.audit.append(
                kind="operation_execution_deferred",
                seal="EXECUTOR",
                principal=operation.principal,
                route=f"product:{operation.capability_id}",
                detail=json.dumps(
                    {"operation_id": operation.id, "outbox_seq": seq},
                    sort_keys=True,
                    separators=(",", ":"),
                ),
            )
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            ...
            "outbox_capacity_remaining": self.outbox.capacity_remaining,
