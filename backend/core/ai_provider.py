...
The application previously depended on the retired Emergent SDK compatibility
shim from request handlers. This module keeps provider-specific SDK objects out
of routes and exposes a small, testable contract for model execution.

Only the OpenAI adapter is activated here today. Additional providers should
implement :class:`ProviderAdapter` and be selected by ``AI_PROVIDER`` rather
than importing vendor SDKs from route modules.
"""
...
def normalize_history(
    ...
    *,
    ...
    Unknown roles, empty messages, and malformed entries are ignored. The
    newest messages are retained when the character budget is exceeded.
    """
...
class ProviderRegistry:
    """Configuration-driven provider selector used by API routes."""
