...
Accepted citation attestations are persisted in EvidenceRegistry. Re-verification
may therefore distinguish historical unbound evidence from truth-eligible evidence.
"""
...
def evidence_item_from_dict(raw: dict[str, Any]) -> EvidenceItem:
    ...
    return EvidenceItem(
        ...
        supports=bool(raw.get("supports", True)), independence_group=independence,
        quality=float(raw.get("quality", raw.get("confidence", 0.0)) or 0.0),
        observed_at=str(raw.get("observed_at") or ""), reproducible=bool(raw.get("reproducible", False)),
        peer_reviewed=bool(raw.get("peer_reviewed", False)), primary=bool(raw.get("primary", False)),
        notes=str(raw.get("notes") or "")[:4000], provenance_verified=provenance_verified,
        preregistered=bool(raw.get("preregistered", False)), data_available=bool(raw.get("data_available", False)),
        code_available=bool(raw.get("code_available", False)), sample_size=_sample_size(raw.get("sample_size")),
        uncertainty_reported=bool(raw.get("uncertainty_reported", False)),
    )


class EpistemicGate:
    def __init__(self, verifier: TruthVerifier | None = None, registry: EvidenceRegistry | None = None,
                 independence: SourceIndependenceAnalyzer | None = None) -> None:
        self.verifier = verifier or TruthVerifier(); self.registry = registry; self.independence = independence
    ...
    def _independence_payload(report: Any | None) -> dict[str, Any] | None:
        ...
        return {"raw_sources": report.raw_sources, "effective_independent_sources": report.effective_independent_sources,
                "unresolved_sources": list(report.unresolved_sources),
                "clusters": [{"id": row.id, "source_ids": list(row.source_ids), "reasons": list(row.reasons),
    ...
    def _validated_rows(self, claim: str, source_rows: Any) -> tuple[tuple[dict[str, Any], ...], tuple[dict[str, Any], ...]]:
        if not isinstance(source_rows, (list, tuple)): return (), ()
        ...
        for raw in source_rows:
            ...
            report = self.citation_integrity.validate(CitationBinding(
                claim=claim, source_id=str(raw.get("source_id") or raw.get("source") or ""),
                locator=str(raw.get("locator") or ""), binding_method=str(binding.get("binding_method") or ""),
                evidence_span=str(binding.get("evidence_span") or ""), supports=bool(raw.get("supports", True)),
                provenance_verified=bool(raw.get("provenance_verified", raw.get("verified_locator", False))),
            ...
            if report.accepted:
                normalized = dict(raw)
                normalized["__citation_binding_attestation_sha256"] = report.attestation_sha256
    ...
    def evaluate(self, finding: dict[str, Any]) -> EpistemicDecision:
        ...
        if not isinstance(generic_raw, (list, tuple)): generic_raw = ()

        evidence_by_claim: dict[str, tuple[EvidenceItem, ...]] = {}
        independence_by_claim: dict[str, Any] = {}; citation_rejections: dict[str, tuple[dict[str, Any], ...]] = {}
        for claim in claims:
            ...
            if self.registry is not None:
                for raw in validated_rows:
                    item = evidence_item_from_dict(raw)
                    self.registry.register(
                        claim, item,
                        citation_binding_attestation_sha256=str(raw.get("__citation_binding_attestation_sha256") or ""),
                    )
                items = self.registry.evidence_for(claim, citation_bound_only=True)
            else:
                items = tuple(evidence_item_from_dict(raw) for raw in validated_rows)
            report = None
            if self.independence is not None:
                items, report = self.independence.collapse(items)
            evidence_by_claim[claim] = items; independence_by_claim[claim] = report

        falsifiability = finding.get("falsifiable") or {}
        if not isinstance(falsifiability, dict): falsifiability = {}
        batch: VerificationBatch = self.verifier.verify_many(claims, evidence_by_claim, falsifiability=falsifiability)
        projection = self.verifier.epistemic_projection(batch); verification: dict[str, dict[str, Any]] = {}
        for group in (batch.verified, batch.provisional, batch.contradicted, batch.unverified, batch.irrelevant_speculation):
            for item in group:
                ...
                if rejected: reasons = list(dict.fromkeys([*reasons, "citation_binding_rejected"]))
                verification[item.claim] = {
                    "state": item.state.value, "empirical_support": item.empirical_support,
                    ...
                    "total_quality": item.total_quality, "mean_quality": item.mean_quality,
                    "reproducibility_signal": item.reproducibility_signal,
                    "independent_replication": item.independent_replication, "falsifiable": item.falsifiable,
                    "reasons": reasons, "citation_rejections": list(rejected),
        ...
        return EpistemicDecision(tuple(projection["verified"]), tuple(projection["provisional"]),
                                 tuple(projection["contradicted"]), tuple(projection["unverified"]),
                                 tuple(projection["irrelevant_speculation"]), verification, _digest(payload))

    @staticmethod
    def hoag_gaps(decision: EpistemicDecision) -> tuple[str, ...]:
        rows: list[str] = []
        rows += [f"PROVISIONAL — requires stronger independent empirical support: {x}" for x in decision.provisional_claims]
        rows += [f"CONTRADICTED — requires resolution before use: {x}" for x in decision.contradicted_claims]
        rows += [f"UNVERIFIED — excluded from authoritative context: {x}" for x in decision.unverified_claims]
        rows += [f"IRRELEVANT SPECULATION — not evidence: {x}" for x in decision.irrelevant_speculation]
        return tuple(rows)
