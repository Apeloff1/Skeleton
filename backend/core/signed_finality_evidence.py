...
def build_signed_finality_evidence(runtime) -> dict[str, Any] | None:
    ...
    evidence = runtime.signed_witnesses.evidence(
        log_id=latest.log_id, tree_size=latest.tree_size, root_sha256=latest.root_sha256, now=when,
    )
...
def verify_signed_finality_evidence(bundle: Mapping[str, Any], *, trusted_witnesses: Mapping[str, Mapping[str, str]],
                                    required_groups: int, max_age_seconds: int,
                                    expected_finality_sha256: str | None = None) -> bool:
    try:
        if int(bundle.get("version", 0)) != 1 or required_groups < 1 or max_age_seconds < 1:
            return False
        finality = bundle.get("finality"); evidence = bundle.get("witness_evidence")
        if not isinstance(finality, Mapping) or not isinstance(evidence, Mapping) or not verify_finality_record(finality):
            return False
        if expected_finality_sha256 is not None and not hmac.compare_digest(str(finality.get("sha256", "")), str(expected_finality_sha256)):
            return False
        ...
        finality_time = finality_time.astimezone(UTC)
        statements = evidence.get("statements")
        if not isinstance(statements, list): return False
        valid_by_witness: dict[str, Mapping[str, Any]] = {}
        for raw in statements:
            ...
            if not public_key or not group or not verify_signed_statement(raw, public_key_b64=public_key, expected_group=group):
                continue
            if raw.get("log_id") != finality.get("log_id") or int(raw.get("tree_size", -1)) != int(finality.get("tree_size", -2)):
                continue
            ...
            if not 0 <= age <= max_age_seconds: continue
            prior = valid_by_witness.get(witness_id)
            if prior is not None and prior.get("root_sha256") != raw.get("root_sha256"): return False
            valid_by_witness[witness_id] = raw
        groups = tuple(sorted({str(trusted_witnesses[w]["independence_group"]) for w in valid_by_witness}))
        if len(groups) < required_groups: return False
        witness_ids = tuple(sorted(valid_by_witness))
        quorum_payload = {
            "log_id": str(finality["log_id"]), "tree_size": int(finality["tree_size"]),
            "root_sha256": str(finality["root_sha256"]), "trusted_receipts": len(valid_by_witness),
            "fresh_receipts": len(valid_by_witness), "stale_receipts": 0,
            "independent_groups": len(groups), "required_groups": int(required_groups),
            "max_age_seconds": int(max_age_seconds), "reached": True, "frozen": False,
            "witness_ids": witness_ids, "groups": groups,
        }
        quorum_sha = _sha(quorum_payload)
        if not hmac.compare_digest(quorum_sha, str(finality.get("witness_quorum_sha256", ""))): return False
        if tuple(finality.get("witness_groups", ())) != groups: return False
        claimed_bundle = str(bundle.get("bundle_sha256", ""))
        raw_payload = {"version": 1, "finality": dict(finality), "witness_evidence": dict(evidence)}
        return hmac.compare_digest(_sha(raw_payload), claimed_bundle)
