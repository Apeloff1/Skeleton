...
is a derived view governed by empirical evidence, source lineage, source
independence, claim dependencies, expiry, contradiction state and re-verification.
History is never rewritten to manufacture certainty; authority changes with proof.
"""
...
class VerifiedCuriosityEngine(CuriosityEngine):
    ...
    def _apply_dependency_gate(self, decision: EpistemicDecision) -> EpistemicDecision:
        accepted: list[str] = []
        remaining = list(decision.verified_claims)
        while remaining:
            progressed = False
        ...
        payload = {
            "verified": tuple(accepted), "provisional": decision.provisional_claims,
        ...
        return EpistemicDecision(
            tuple(accepted), decision.provisional_claims, decision.contradicted_claims,
    ...
    def _resolve_calibration(self, claim: str, state: VerificationState, attestation: str) -> int:
        # Insufficient evidence is not evidence of falsity. Forecasts resolve only
        # when the verifier reaches a positive VERIFIED or empirical CONTRADICTED state.
        if state not in {VerificationState.VERIFIED, VerificationState.CONTRADICTED}:
            return 0
        outcome = state == VerificationState.VERIFIED
        resolved = 0
        for forecast in self.calibration.snapshot():
            if forecast.claim == claim and forecast.outcome is None:
                self.calibration.resolve(
                    forecast.id, outcome=outcome,
    ...
    def accept_finding(self, inquiry: Inquiry, finding: dict[str, Any]) -> KnowledgeRecord:
        ...
        for claim in decision.verified_claims:
            row = decision.verification.get(claim, {})
            attestation = str(row.get("attestation_sha256") or "")
            if len(attestation) == 64:
                self._resolve_calibration(claim, VerificationState.VERIFIED, attestation)
        for claim in decision.contradicted_claims:
            row = decision.verification.get(claim, {})
            attestation = str(row.get("attestation_sha256") or "")
            if len(attestation) == 64:
                self._resolve_calibration(claim, VerificationState.CONTRADICTED, attestation)
        ...
        verified_evidence = self._verified_evidence(decision); verified_count = len(decision.verified_claims)
        confidence = 0.0
        if verified_count:
            confidence = min(
                self.verifier.verify_claim(
                    claim, self._active_evidence(claim),
                    falsifiable=(finding.get("falsifiable") or {}).get(claim)
                    if isinstance(finding.get("falsifiable"), dict) else None,
                ).mean_quality
                for claim in decision.verified_claims
            )
        ...
        with self._lease.acquire():
            ...
            if raw is not None:
                ...
                raw["unresolved_count"] = len(gaps) + len(contradictions); topics[inquiry.subject] = raw
    ...
    def reverify_claim(self, claim: str) -> dict[str, Any]:
        ...
        evidence_ids = [r.id for r in self.evidence_registry.records_for(claim) if not r.retracted]
        effective_state = result.state.value
        ...
        if result.state == VerificationState.VERIFIED and not blockers:
            ...
            if blockers:
                effective_state = "dependency_blocked"; reasons.append("non_authoritative_dependency")
            if self.truth_ledger.get(claim) is not None:
        ...
        return {
            "claim": claim, "state": effective_state,
            "authoritative": self.truth_ledger.authoritative(claim),
            ...
            "contradiction": {
                ...
                "required_actions": list(contradiction.required_actions),
                "attestation_sha256": contradiction.attestation_sha256,
            },
        }

    def reverify_expired(self) -> dict[str, Any]:
        expired = self.truth_ledger.expired_claims()
        results = [self.reverify_claim(claim) for claim in expired]
    ...
    def retract_source(self, source_id: str, reason: str, *, cascade: bool = True) -> dict[str, Any]:
        ...
        evidence = self.evidence_registry.retract_sources(
            affected_sources, f"source retraction: {source_id} — {reason}",
        )
        results = [self.reverify_claim(claim) for claim in evidence["claims"]]
        dependent_claims = self._revoke_dependents(
            list(evidence["claims"]), f"upstream evidence retracted: {source_id}",
        )
        return {
            "source_id": source_id,
            "affected_sources": list(affected_sources),
            "retracted_evidence_records": list(evidence["record_ids"]),
            "affected_claims": list(evidence["claims"]),
            "dependent_claims_revoked": list(dependent_claims),
    ...
    def resolve_source_lineage(self, source_id: str, *, source_kind: str, locator: str = "",
        ...
        node = self.source_lineage.resolve_legacy(
            source_id,
        ...
        affected_claims = tuple(sorted({
            record.claim
            for record in self.evidence_registry.records_for_sources((source_id,), include_retracted=False)
        }))
    ...
    def contradiction_status(self, claim: str) -> dict[str, Any]:
        ...
        return {
            "claim": report.claim,
            "state": report.state,
            "support_groups": list(report.support_groups),
            "contradiction_groups": list(report.contradiction_groups),
            "support_quality": report.support_quality,
            "contradiction_quality": report.contradiction_quality,
            "methodological_conflict": report.methodological_conflict,
    ...
    def compare_claims(self, left: str, right: str) -> dict[str, Any]:
        match = self.claim_identity.compare(left, right)
        return asdict(match)
    ...
    def record_forecast(self, *, claim: str, probability: float, forecaster: str,
        ...
        return asdict(self.calibration.forecast(
    ...
    def verification_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "source_lineage": self.source_lineage.stats(),
            "lineage_backfill": self._lineage_backfill,
            "truth_ledger": self.truth_ledger.stats(),
            "claim_dependencies": self.claim_dependencies.stats(),
            "calibration": {**self.calibration.stats(), "metrics": self.calibration.metrics()},
            "knowledge": knowledge,
            "verification_policy": {
