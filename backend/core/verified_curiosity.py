...
class VerifiedCuriosityEngine(CuriosityEngine):
    ...
    def _orientation_summary(decision: EpistemicDecision) -> str:
        if decision.verified_claims: return "Verified knowledge: " + " ".join(decision.verified_claims[:8])
    ...
    def backfill_source_lineage(self) -> dict[str, int]:
        ...
        for record in self.evidence_registry.snapshot(include_retracted=True):
            ...
            if self.source_lineage.get(item.source_id) is not None:
                existing += 1; continue
            self.source_lineage.register(item.source_id, source_kind=f"{LEGACY_UNRESOLVED_PREFIX}{item.kind.value}", locator=item.locator)
            created += 1
    ...
    def _register_lineage(self, finding: dict[str, Any]) -> None:
        ...
        for _ in range(len(pending) + 1):
            ...
            for source_id, raw in list(pending.items()):
                ...
                self.source_lineage.register(source_id, source_kind=str(raw.get("kind") or "unknown"),
                    locator=str(raw.get("locator") or ""), parent_ids=parents,
    ...
    def _revoke_dependents(self, claims: tuple[str, ...] | list[str], reason: str) -> tuple[str, ...]:
        ...
        for claim in claims: affected.update(self.claim_dependencies.dependents(claim, recursive=True))
    ...
    def _active_evidence(self, claim: str):
        rows = self.evidence_registry.evidence_for(claim); collapsed, _ = self.independence.collapse(rows); return collapsed
    ...
    def verification_status(self) -> dict[str, Any]:
        ...
        return {"truth_gated": True, "speculation_authoritative": False, "model_consensus_is_empirical_evidence": False,
            "evidence_registry": evidence, "source_lineage": self.source_lineage.stats(), "lineage_backfill": self._lineage_backfill,
            "truth_ledger": self.truth_ledger.stats(), "claim_dependencies": self.claim_dependencies.stats(), "knowledge": knowledge,
