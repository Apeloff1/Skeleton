...
_STOPWORDS = {
    ...
    "would", "could", "should", "have", "has", "had", "your", "you", "our", "are", "was", "were", "will",
    "make", "build", "create", "please", "tell", "show", "give", "more", "keep", "continue", "also", "than",
}
...
def _subject(prompt: str, keys: tuple[str, ...]) -> str:
    ...
    # Prefer a compact keyword subject while retaining enough meaning for niche prompts.
    if keys:
        return " / ".join(keys[:5])[:240]
    return cleaned[:240] or "general inquiry"
...
class CuriosityEngine:
    ...
    def _load_frontier(self) -> dict[str, dict[str, Any]]:
        try:
            envelope = json.loads(self.frontier_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise CuriosityIntegrityError("curiosity frontier unreadable") from exc
        if envelope.get("version") != FRONTIER_VERSION:
            raise CuriosityIntegrityError("unsupported curiosity frontier version")
        topics = envelope.get("topics")
        digest = envelope.get("sha256")
        if not isinstance(topics, dict) or not isinstance(digest, str):
            raise CuriosityIntegrityError("curiosity frontier malformed")
        if not hmac.compare_digest(digest, self._frontier_digest(topics)):
            raise CuriosityIntegrityError("curiosity frontier checksum mismatch")
    ...
    def observe_prompt(self, prompt: str, *, user_scope: str = "default", observed_at: str | None = None) -> PromptSignal:
        prompt = " ".join(str(prompt).split()).strip()
        if not prompt:
            raise ValueError("prompt cannot be blank")
        keys = _keywords(prompt)
        subject = _subject(prompt, keys)
        ...
        with self._lease.acquire():
            ...
            raw = topics.get(subject, {})
            topic = FrontierTopic(
                subject=subject,
                keywords=list(raw.get("keywords", ())),
                prompt_count=int(raw.get("prompt_count", 0)),
                unresolved_count=int(raw.get("unresolved_count", 0)),
                research_count=int(raw.get("research_count", 0)),
                last_prompt_at=str(raw.get("last_prompt_at", "")),
                last_researched_at=str(raw.get("last_researched_at", "")),
                last_record_id=raw.get("last_record_id"),
                priority_bias=float(raw.get("priority_bias", 0.0)),
            )
            topic.prompt_count += 1
            topic.last_prompt_at = stamp
            topic.keywords = list(dict.fromkeys([*keys, *topic.keywords]))[:24]
            topics[subject] = asdict(topic)
    ...
    def _score_topic(self, topic: FrontierTopic, *, now: float) -> tuple[float, str]:
        ...
        coverage = min(1.0, len(existing) / 6.0)
        mean_conf = sum(r.confidence for r in existing) / len(existing) if existing else 0.0
        unresolved = sum(len(r.questions) + len(r.contradictions) for r in existing) + topic.unresolved_count
        prompt_strength = min(1.0, math.log2(topic.prompt_count + 1) / 3.0)
        novelty = 1.0 - min(1.0, coverage * max(mean_conf, 0.25))
        last = _parse_time(topic.last_researched_at)
        staleness = 1.0 if not last else min(1.0, max(0.0, now - last) / (7 * 86400.0))
        gap_pressure = min(1.0, unresolved / 8.0)
        fatigue = min(0.6, topic.research_count * 0.08)
        score = 0.30 * prompt_strength + 0.28 * novelty + 0.22 * gap_pressure + 0.20 * staleness + topic.priority_bias - fatigue
        reason = f"prompt={prompt_strength:.2f};novelty={novelty:.2f};gaps={gap_pressure:.2f};stale={staleness:.2f};fatigue={fatigue:.2f}"
        return max(0.0, round(score, 6)), reason
    ...
    def frontier(self, *, limit: int = 20, now: float | None = None) -> tuple[tuple[FrontierTopic, float, str], ...]:
        ...
        with self._lease.acquire():
            raw_topics = self._load_frontier()
        ranked: list[tuple[FrontierTopic, float, str]] = []
        for raw in raw_topics.values():
            topic = FrontierTopic(**raw)
            score, reason = self._score_topic(topic, now=now)
            ranked.append((topic, score, reason))
    ...
    def next_inquiry(self, *, minimum_score: float = 0.18, now: float | None = None) -> Inquiry | None:
        ...
        if score < minimum_score:
            return None
        context = self.fabric.search(topic.subject, limit=6)
        unresolved = [q for record in context for q in (*record.contradictions, *record.questions)]
        questions: list[str] = []
        for question in unresolved:
            if question not in questions:
                questions.append(question)
        defaults = [
            f"What are the foundational concepts and strongest evidence for {topic.subject}?",
            f"What are the most important unresolved disagreements or failure modes in {topic.subject}?",
            f"What has changed recently or is commonly misunderstood about {topic.subject}?",
            f"Which adjacent concepts would most improve practical reasoning about {topic.subject}?",
        ]
    ...
    def _evidence(raw: Iterable[dict[str, Any]]) -> tuple[EvidenceRef, ...]:
        ...
        for item in raw:
            ...
            out.append(EvidenceRef(
                source=str(item["source"])[:1000], locator=str(item.get("locator", ""))[:2000],
                confidence=conf, observed_at=str(item.get("observed_at", ""))[:100],
            ))
    ...
    def accept_finding(self, inquiry: Inquiry, finding: dict[str, Any]) -> KnowledgeRecord:
        ...
        if not summary:
            raise ValueError("research finding requires a summary")
        evidence = self._evidence(finding.get("evidence") or ())
        claims = tuple(str(x) for x in (finding.get("claims") or ()))
        # Unsupported claims are allowed only as hypotheses/gaps, never elevated to high confidence.
        requested_conf = max(0.0, min(1.0, float(finding.get("confidence", 0.5))))
        confidence = min(requested_conf, 0.45) if claims and not evidence else requested_conf
        previous = self.fabric.search(inquiry.subject, limit=12)
        previous_claims = {c.casefold() for r in previous for c in r.claims}
        novel_claims = sum(1 for claim in claims if claim.casefold() not in previous_claims)
        ...
        record = self.fabric.publish(
            ...
            questions=tuple(str(x) for x in (finding.get("questions") or ())),
            contradictions=tuple(str(x) for x in (finding.get("contradictions") or ())),
            evidence=evidence, tags=tuple(inquiry.keywords) + tuple(str(x) for x in (finding.get("tags") or ())),
            confidence=confidence, novelty=novelty, parent_prompt_id=inquiry.id,
        )
        with self._lease.acquire():
            topics = self._load_frontier()
            ...
            if raw is not None:
                ...
                raw["unresolved_count"] = len(record.questions) + len(record.contradictions)
    ...
    async def run_once(self, researcher: Researcher, *, minimum_score: float = 0.18) -> dict[str, Any]:
        ...
        if inquiry is None:
            return {"status": "idle", "reason": "no inquiry above curiosity threshold"}
        context = self.fabric.orientation_pack(inquiry.subject, limit=8)
        result = researcher(inquiry, context)
        if inspect.isawaitable(result):
            result = await result
        record = self.accept_finding(inquiry, result)
        return {
            "status": "learned", "inquiry_id": inquiry.id, "subject": inquiry.subject,
            "record_id": record.id, "digest": record.digest, "score": inquiry.score,
            "surfaces": ("wiki", "hoag", "newsroom", "orientation-room"),
        }
    ...
    def boost(self, subject: str, delta: float = 0.15) -> bool:
        with self._lease.acquire():
            ...
            if raw is None:
                return False
            raw["priority_bias"] = max(-0.5, min(0.75, float(raw.get("priority_bias", 0.0)) + delta))
            topics[subject] = raw; self._write_frontier(topics)
            return True
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "topics": len(topics),
            "prompt_signals": sum(int(x.get("prompt_count", 0)) for x in topics.values()),
            "research_cycles": sum(int(x.get("research_count", 0)) for x in topics.values()),
            "frontier_sha256": self._frontier_digest(topics),
