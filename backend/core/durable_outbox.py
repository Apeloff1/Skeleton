...
class DurableOutbox:
    ...
    def __init__(self, directory: str | os.PathLike[str], cap: int = 4096) -> None:
        ...
        self.meta_path = self.directory / "outbox.meta.json"
        self.lock_path = self.directory / ".outbox.lock"
        self.cap = cap
        self._lock = threading.RLock()
        with self._guard(refresh=False):
            self._entries = self._restore()
            inferred = max((entry.seq for entry in self._entries), default=0) + 1
    ...
    def _load_next_seq(self, *, default: int) -> int:
        if not self.meta_path.exists():
            return default
        try:
            envelope = json.loads(self.meta_path.read_text(encoding="utf-8"))
            payload = envelope["payload"]
            digest = envelope["sha256"]
            version = int(payload["version"])
            next_seq = int(payload["next_seq"])
        except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise OutboxIntegrityError("outbox sequence metadata is unreadable") from exc
        if version != self.META_VERSION or next_seq <= 0 or not isinstance(digest, str):
            raise OutboxIntegrityError("outbox sequence metadata is malformed")
        if not hmac.compare_digest(self._digest(payload), digest):
            raise OutboxIntegrityError("outbox sequence metadata checksum mismatch")
        return next_seq

    def _persist_next_seq(self, next_seq: int) -> None:
        payload = {"version": self.META_VERSION, "next_seq": next_seq}
        ...
        try:
            with temp.open("x", encoding="utf-8") as handle:
                handle.write(self._canonical(envelope).decode("utf-8"))
                handle.flush()
    ...
    def journal(self, collection: str, payload: dict[str, Any]) -> OutboxEntry:
        ...
        with self._guard():
            ...
            entry = OutboxEntry(seq=self._next_seq, collection=collection, payload=dict(payload), journaled_at=datetime.now(UTC).isoformat())
    ...
    async def confirm_one(self, seq: int, confirmer: Confirmer) -> bool:
        ...
        with self._guard():
            if not any(item.seq == seq for item in self._entries): return True
    ...
    def health(self) -> dict[str, Any]:
        with self._guard():
            return {
                "pending": len(self._entries), "capacity": self.cap,
                "capacity_remaining": max(0, self.cap - len(self._entries)),
                "next_sequence": self._next_seq, "sequence_meta_version": self.META_VERSION,
