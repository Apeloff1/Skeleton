...
class NativeProductExecutors:
    def __init__(self, receipt_store: ExecutionReceiptStore, *, sessions: RuntimeSessionManager | None = None) -> None:
    ...
    def _already_complete(self, operation: AdmittedOperation, executor: str) -> bool:
        ...
        if existing is None:
            return False
        if existing.capability_id != operation.capability_id or existing.action != operation.action or existing.executor != executor:
    ...
    def _write(self, operation: AdmittedOperation, executor: str, result: dict[str, Any]) -> bool:
        self.receipts.write(operation_id=operation.id, capability_id=operation.capability_id,
                            action=operation.action, executor=executor, result=result)
        return True
    ...
    def create_project(self, operation: AdmittedOperation, payload: dict[str, Any]) -> bool:
        ...
        descriptor = {"project_id": operation.id, "title": title[:200],
                      ...
                      "owner": operation.principal, "state": "created"}
    ...
    def create_world(self, operation: AdmittedOperation, payload: dict[str, Any]) -> bool:
        ...
        return self._write(operation, executor, {"world": world, "seed": getattr(cfg, "seed", None),
    ...
    def register_into(self, registry: ProductExecutorRegistry) -> ProductExecutorRegistry:
        registry.register("studio", "project.create", self.create_project, name="native.studio.project.create",
                          version=1, effect_class="state", replay_safe=True)
        ...
        registry.register("playables", "runtime.sessions", self.runtime_sessions, name="native.playables.runtime.sessions",
                          version=1, effect_class="query", replay_safe=True)
        return registry


def build_default_executor_registry(receipt_store: ExecutionReceiptStore, *, sessions: RuntimeSessionManager | None = None) -> tuple[ProductExecutorRegistry, NativeProductExecutors]:
