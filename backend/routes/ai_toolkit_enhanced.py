...
async def list_code_reviews(limit: int = 30):
    """Surface stored AI code reviews (code_reviews was written but never read)."""
    rows = await ai_toolkit_db.code_reviews.find({}, {"_id": 0}).sort("created_at", -1).to_list(min(limit, 100))
