...
Endpoints
---------
GET  /api/godot-engine/status            — binary availability, version, source
POST /api/godot-engine/projects          — scaffold a new Godot project
...
async def godot_status() -> dict:
    """Report whether the Godot binary is present and probe its version."""
    status = binary_status()
    if not status.get("available"):
        return status
    try:
        status["version"] = get_binary().probe()
...
async def create_project(req: ProjectCreateRequest) -> dict:
    ...
    result = scaffold_project(spec, _PROJECTS_ROOT)
...
async def submit_job(req: JobSubmitRequest) -> dict:
    ...
    project_dir = None
    if req.project_slug:
        project_dir = _PROJECTS_ROOT / req.project_slug
        if not project_dir.is_dir():
            raise HTTPException(404, f"no project named {req.project_slug!r}")
    kwargs: dict = {}
    if req.kind == "export":
