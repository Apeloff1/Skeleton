...
_db = _mongo[os.environ.get('DB_NAME', 'codedock')]


async def _search_knowledge_vault(query: str, language: str = "") -> str:
    """Search the pre-seeded knowledge vault for relevant context.
    Returns matching entries to inject into the LLM prompt, reducing external knowledge needs."""
    query_lower = query.lower()
    keywords = [w for w in query_lower.split() if len(w) > 2]

    # Build comprehensive search — check tags, category, topic, and content
    or_conditions = []
    ...
    if language:
        or_conditions.append({"category": language.lower()})
        or_conditions.append({"tags": language.lower()})

    if not or_conditions:
        return ""

    entries = await _db.knowledge_vault.find(
        {"$or": or_conditions}, {"_id": 0, "topic": 1, "content": 1}
    ).limit(5).to_list(5)
    ...
    context_parts = []
    for e in entries:
        context_parts.append(f"### {e['topic']}\n{e['content']}")
    return "\n\n---\n\n".join(context_parts)
...
class JeevesRequest(BaseModel):
    ...
    mastery_dna: Optional[Dict[str, float]] = Field(
        None,
        description="Optional 100-slider Jeeves DNA cockpit payload (keys `jv_<group>_<slot>`).",
    )
...
async def call_jeeves(
    prompt: str,
    personality: str,
    skill_level: str,
    session_id: str = None,
    language: str = "python",
    mastery_dna: Optional[Dict[str, float]] = None,
) -> str:
    """LLM call for Jeeves. Hardened with timeout + bounded retry + safe error.

    When a ``mastery_dna`` cockpit payload is supplied, the translator turns
    every drifted slider into a directive block that's appended to the system
    prompt — Jeeves's "code style / security paranoia / reviewer voice"
    knobs reach the LLM verbatim.
    """
    import asyncio
    from .dna_translator_core import translate as translate_dna
    from .dna_domains import JEEVES_DOMAIN

    if not EMERGENT_LLM_KEY:
        log.error("call_jeeves: EMERGENT_LLM_KEY is empty")
        raise HTTPException(status_code=503, detail="AI provider is not configured")

    # Bound the user-supplied prompt to keep token usage under control.
    if len(prompt) > 40_000:
        raise HTTPException(status_code=413, detail="prompt exceeds 40k char limit")

    vault_context = await _search_knowledge_vault(prompt, language)
    vault_injection = ""
    if vault_context:
        vault_injection = f"\n\n[INTERNAL KNOWLEDGE BASE — Use this as primary reference]\n{vault_context}\n[END KNOWLEDGE BASE]\n"

    dna_block = translate_dna(mastery_dna, JEEVES_DOMAIN)
    dna_injection = f"\n\n{dna_block}\n" if dna_block else ""
...
IMPORTANT: Keep responses concise and focused. Use the internal knowledge base above when relevant.
Max response length: 600 words. Be direct. Use code examples when helpful but keep them short.
Always be helpful, accurate, and adapt to the user's level."""

    timeout_s = float(os.environ.get('JEEVES_AI_TIMEOUT_S', '60'))
    attempts = max(1, int(os.environ.get('JEEVES_AI_MAX_RETRIES', '1')) + 1)
...
Please help with this query. Remember to adapt your response to a {request.skill_level} level developer."""
    ...
    try:
        response = await call_jeeves(
            prompt,
            request.personality,
            request.skill_level,
            request.session_id,
            request.language,
            mastery_dna=request.mastery_dna,
        )
        ...
        log.exception("ask_jeeves: unhandled error")
        raise HTTPException(status_code=500, detail="Jeeves stumbled. Please try again.")
