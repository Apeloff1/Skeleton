...
@router.get("/therapeutic/log")
async def list_therapeutic_interactions(limit: int = 30):
    """Surface Jeeves EQ therapeutic interactions (was written but never read)."""
    rows = await eq_db.therapeutic_interactions.find({}, {"_id": 0}).sort("created_at", -1).to_list(min(limit, 100))
