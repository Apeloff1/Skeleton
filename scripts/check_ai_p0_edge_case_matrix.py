#!/usr/bin/env python3
"""Validate W00-W11 edge/historical construction obligations."""
...
def validate() -> list[str]:
    errors: list[str] = []
    for path in (MATRIX, CATALOG, MASTER, HUMAN):
        if not path.is_file():
    ...
    for pkg in packages:
        pid = pkg.get("id", "?")
        for field in ("exact_paths", "historical_ids", "edge_case_ids", "obscure_ids", "invariants", "test_targets", "acceptance"):
            values = pkg.get(field)
            if not isinstance(values, list) or not values:
                errors.append(f"{pid}: {field} must be a non-empty list")
        refs = pkg.get("historical_ids", []) + pkg.get("edge_case_ids", []) + pkg.get("obscure_ids", [])
    ...
    if len(referenced) < 100:
        errors.append(f"W00-W11 should reference at least 100 unique catalog cases; found {len(referenced)}")
