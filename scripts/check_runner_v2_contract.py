...
def _workflow_findings(text: str) -> list[str]:
    ...
    required_markers = (
        ...
        "vars.PR_RUNNER_REQUIRE_PROTECTED_BASE || 'false'",
...
def _cross_file_findings() -> list[str]:
    ...
    scheduler = (PACKAGE / "runner_scheduler.py").read_text(encoding="utf-8")
    ...
    checks = (
        ...
        (
            "class GitHubHTTPError" in transport
            and "definitive_mutation_rejection" in transport
            and "except GitHubHTTPError as exc" in transaction,
            "mutation transport must distinguish definitive HTTP rejection from ambiguous dispatch",
        ),
        (
        ...
        (
            'reasons.append("rate_limit_unknown")' in scheduler,
            "mutation admission must fail closed when rate-limit evidence is unknown",
        ),
        (
        ...
        (
            'UNCERTAIN = "uncertain"' in contracts,
            "uncertain mutation state must be explicit",
        ),
        (
            'row["status"] in {"succeeded", "uncertain"}'
            in (PACKAGE / "index.py").read_text(encoding="utf-8"),
            "uncertain durable claims must be non-replayable",
        ),
        (
            "MutationState.UNCERTAIN" in transaction
            and "uncertain=True" in transaction,
            "ambiguous merge dispatch must quarantine the mutation claim",
        ),
        (
            "MutationState.UNCERTAIN" in engine,
            "runner engine must surface uncertain mutations",
        ),
        (
