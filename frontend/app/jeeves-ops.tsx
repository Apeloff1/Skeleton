/**
 * /jeeves-ops — Operator console for the Jeeves service surface.
 *
 * This screen is intentionally self-contained so it can remain useful even
 * when one of the richer Jeeves feature modules fails to mount. It probes the
 * same public service endpoints as the Powerhouse Hub, records request latency,
 * classifies health, and gives operators a focused retry/inspection surface.
 */
...
function payloadRows(data: unknown): { label: string; value: string }[] {
  if (Array.isArray(data)) {
    return data.slice(0, 8).map((item, index) => ({
      label: `Item ${index + 1}`,
      value: typeof item === 'string' ? item : JSON.stringify(item).slice(0, 180),
    }));
  }
  if (!data || typeof data !== 'object') {
  ...
  return Object.entries(data as Record<string, unknown>)
    ...
    .map(([key, value]) => {
      ...
      else rendered = JSON.stringify(value).slice(0, 180);
...
export default function JeevesOpsScreen() {
        ...
        <View style={styles.summaryCard}>
          ...
          <View style={styles.meterTrack}>
            <View
              style={[
                styles.meterFill,
                { width: `${stats.total ? Math.round((stats.healthy / stats.total) * 100) : 0}%` },
              ]}
            />
          </View>
          ...
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity
                ...
                onPress={() => openService && checkService(openService)}
                accessibilityRole="button"
              >
...
const styles = StyleSheet.create({
  ...
  title: { color: '#F8FAFC', fontSize: 19, fontWeight: '900', marginTop: 1 },
  ...
  endpointText: { color: '#A78BFA', fontSize: 10, fontFamily: 'monospace', marginTop: 3, marginBottom: 12 },
