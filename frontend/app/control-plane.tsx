...
import {
  ...
  executePendingProductOperations,
  executeProductOperation,
...
export default function ControlPlaneRoute() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [statusResult, pendingResult, auditResult, receiptResult] = await Promise.all([
      getProductControlStatus(cleanToken), getPendingProductOperations(cleanToken),
      getProductAuditHistory(cleanToken, 25), getExecutionReceipts(cleanToken, 25),
    ]);
    if (!statusResult.ok || !statusResult.data) setError(statusResult.error || `Status unavailable (${statusResult.status})`);
    else setStatus(statusResult.data);
  ...
  const executeOne = useCallback(async (seq: number) => {
    setExecuting(seq); setError(null);
    const result = await executeProductOperation(seq, token.trim());
    if (!result.ok) setError(result.error || 'Execution failed');
    await refresh(); setExecuting(null);
  }, [refresh, token]);

  const executeAll = useCallback(async () => {
    setExecuting('all'); setError(null);
    const result = await executePendingProductOperations(token.trim(), 64);
    if (!result.ok) setError(result.error || 'Batch execution failed');
    await refresh(); setExecuting(null);
  }, [refresh, token]);
      ...
      <ScrollView contentContainerStyle={styles.page} refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} />}>
        ...
        {status ? <>
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.muted}>{status.governance.charters.length} charters · {status.governance.edicts.length} edicts · policy v{status.policy_version}</Text>
            {status.governance.charters.map((charter) => <View key={charter.id} style={styles.row}><View style={{ flex: 1 }}>
              <Text style={styles.rowTitle}>{charter.domain}</Text><Text style={styles.muted}>{charter.rules.length} rules · {charter.amendments} amendments</Text>
            </View></View>)}
          </View>
        </> : null}
        ...
        <View style={styles.card}>
          ...
          {(pending?.operations ?? []).map((operation) => <View key={operation.operation_id} style={styles.row}>
            ...
            {operation.executor_bound ? <TouchableOpacity style={styles.execButton} onPress={() => executeOne(operation.outbox_seq)} disabled={executing !== null}><Text style={styles.smallButtonText}>{executing === operation.outbox_seq ? '…' : 'Execute'}</Text></TouchableOpacity> : null}
          </View>)}
        </View>
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: '#080A0F' }, page: { padding: 18, paddingBottom: 48, gap: 14 }, back: { color: '#99A7FF', fontWeight: '800', paddingVertical: 5 },
  hero: { borderWidth: 1, borderColor: '#232838', borderRadius: 22, padding: 20, backgroundColor: '#0D111A' }, eyebrow: { color: '#8BD3A7', fontSize: 10, fontWeight: '900', letterSpacing: 1.8 },
  title: { color: '#F8FAFC', fontSize: 28, fontWeight: '900', marginTop: 7 }, description: { color: '#AAB3C5', lineHeight: 20, marginTop: 7 }, field: { gap: 7 },
  label: { color: '#8B96A8', fontSize: 11, fontWeight: '800' }, input: { color: '#F8FAFC', borderWidth: 1, borderColor: '#283044', borderRadius: 12, backgroundColor: '#0F141F', paddingHorizontal: 12, paddingVertical: 11 },
  refreshButton: { minHeight: 46, borderRadius: 12, backgroundColor: '#5364D8', alignItems: 'center', justifyContent: 'center' }, refreshText: { color: '#fff', fontWeight: '900' },
