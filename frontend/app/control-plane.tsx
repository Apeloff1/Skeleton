...
import { AuditEntry, ControlPlaneStatus, ExecutionReceipt, PendingOperationResponse, executePendingProductOperations, executeProductOperation, getExecutionReceipts, getPendingProductOperations, getProductAuditHistory, getProductControlStatus } from '../src/product/productControlClient';
...
export default function ControlPlaneRoute() {
  ...
  const refresh = useCallback(async () => {
    ...
    const [statusResult, pendingResult, auditResult, receiptResult] = await Promise.all([
      getProductControlStatus(cleanToken), getPendingProductOperations(cleanToken), getProductAuditHistory(cleanToken, 25), getExecutionReceipts(cleanToken, 25),
    ]);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.page} refreshControl={<RefreshControl refreshing={loading} onRefresh={refresh} />}>
    ...
    {status ? <>
      <View style={styles.metrics}><Metric label="Posture" value={status.assurance.posture.toUpperCase()} /><Metric label="Ready" value={`${status.readiness.ready_pct}%`} /><Metric label="Hard fails" value={String(status.assurance.hard_failures)} /><Metric label="Evidence gaps" value={String(status.lifecycle.evidence_gaps)} /></View>
