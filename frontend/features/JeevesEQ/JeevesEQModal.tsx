...
export const JeevesEQModal: React.FC<JeevesEQModalProps> = ({
  ...
  const fetchEQData = useCallback(async () => {
    ...
    try {
      const [infoRes, profileRes, loadRes, reminderRes] = await Promise.all([
        apiFetch(`${API_URL}/api/jeeves-eq/info`),
        apiFetch(`${API_URL}/api/jeeves-eq/psychology-profile/${userId}`),
        apiFetch(`${API_URL}/api/jeeves-eq/cognitive-load-check?user_id=${userId}&session_duration_minutes=30&new_concepts_introduced=2&error_count=3&last_break_minutes_ago=25`),
        apiFetch(`${API_URL}/api/jeeves-eq/wellness-reminder`)
      ]);
  ...
  const detectEmotion = useCallback(async (recentActions: any[]) => {
    try {
      const res = await apiFetch(`${API_URL}/api/jeeves-eq/detect-emotion?user_id=${userId}`, {
        method: 'POST',
