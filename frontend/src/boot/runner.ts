...
function withTimeout<T>(promise: Promise<T>, ms: number, signal?: AbortSignal): Promise<TimeoutResult<T>> {
...
export class BootRunner {
  ...
  private phaseProgress(): { 0: number; 1: number; 2: number } {
    ...
    for (const phase of [0, 1, 2] as const) {
      ...
      const okWeight = stages.filter(state => state.status === 'ok').reduce((sum, state) => sum + state.weight, 0);
  ...
  snapshot(): RunnerSnapshot {
    ...
    const okWeight = list.filter(state => state.status === 'ok').reduce((sum, state) => sum + state.weight, 0);
    const phaseDone = { 0: false, 1: false, 2: false } as { 0: boolean; 1: boolean; 2: boolean };
    for (const phase of [0, 1, 2] as const) {
      const stages = list.filter(state => state.phase === phase);
      phaseDone[phase] = stages.length > 0 && stages.every(state => state.status !== 'pending' && state.status !== 'running');
    }
    return {
  ...
  private async runOne(def: BootStageDef): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      trail.add('boot', `stage_attempt_failed ${def.id}`, { attempt, max: maxAttempts, reason: note }, 'warn');
    ...
    trail.add('boot', `stage_failed ${def.id}`, { reason: state.error, ms: state.durationMs, attempts: maxAttempts }, 'error');
    this.emit();
  }
  ...
  run(): Promise<RunnerSnapshot> {
    ...
    this.runPromise = Promise.allSettled(Object.values(this.resolvedTasks)).then(() => this.snapshot());
