...
export const STAGES: BootStageDef[] = [
  ...
  // ── Phase 1: diagnostics + network preparation ─────────────────────────
  {
  ...
  },
  {
    ...
    run: async (signal) => {
      if (signal?.aborted) return { ok: false, note: 'aborted' };
      // Startup only needs a bounded signal that the backend is reachable.
      // A cold or offline backend must never delay local interactivity.
