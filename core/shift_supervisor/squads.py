...
def safe_squad_capacity(
    ...
    *,
    ...
    eligible = sum(
        ...
        and worker.overtime_minutes < max(0, overtime_soft_limit_minutes)
    )
...
class SquadCoordinator:
                ...
                worker_meta.update(
                    {
                        ...
                        "squad_assignment_count": self._nonnegative_int(worker_meta.get("squad_assignment_count")) + 1,
                    }
                )
    ...
    def renew(
        ...
        *,
        ...
        moment = (now or datetime.now(timezone.utc)).astimezone(timezone.utc)
        extend = self.default_lease_minutes if minutes is None else max(5, min(int(minutes), 240))
        with self.store._lock:  # noqa: SLF001
    ...
    def _reclaim_expired_locked(self, now: datetime) -> list[str]:
        ...
        for item in list(self.store._items.values()):  # noqa: SLF001
            if item.status not in {"assigned", "working", "blocked"} or not str(item.owner or "").startswith("squad-"):
                continue
            try:
                lease = self._lease_from_item(item)
            except ValueError:
            ...
            meta = dict(item.metadata)
            meta["squad_lease_history"] = history[-16:]
            meta["lease_expiry_count"] = self._nonnegative_int(meta.get("lease_expiry_count")) + 1
            meta.pop("squad_lease", None)
            item.metadata = meta
            item.owner = None
            item.status = "queued"
    ...
    def _eligible_workers_locked(self, team: str, allowed_workers: set[str] | None) -> list[WorkerState]:
        ...
        for worker in self.store._workers.values():  # noqa: SLF001
            if worker.team != team or worker.status != "idle" or worker.current_task_id is not None:
                continue
    ...
    def _select_role_members(self, workers: Sequence[WorkerState]) -> dict[str, WorkerState]:
    ...
    @classmethod
    def _worker_role_score(cls, worker: WorkerState, role: str) -> tuple[int, int, int, str]:
        preferred = worker.metadata.get("preferred_squad_roles", [])
        preferred_roles = {str(value) for value in preferred} if isinstance(preferred, list) else set()
    ...
    def _lease_from_item(cls, item: PlanItem) -> SquadLease:
        ...
        if set(members) != set(SQUAD_ROLES) or len(set(members.values())) != SQUAD_SIZE:
            raise ValueError("squad lease must contain four distinct canonical roles")
        try:
