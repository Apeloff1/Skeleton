...
under the reserved `retrieved_context` key. The retrieval request participates
in idempotency identity, and execution provenance records only source identifiers
and a content digest, never the retrieved document text.
