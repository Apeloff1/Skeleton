...
| API payload/header/rate-limit/error-redaction contracts | canonical runtime middleware and tests | request-body framing tests, `HeaderBoundMiddleware` outer-edge enforcement, rate-limit reliability, and error-redaction regressions | Canonical | Content-type validation and the remaining SSRF/network review remain tracked in #540. |
