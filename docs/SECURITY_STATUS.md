...
| API payload/header/rate-limit/error-redaction contracts | canonical runtime middleware and tests | request-body framing tests, bounded structural `Content-Type` validation, `HeaderBoundMiddleware` outer-edge enforcement, rate-limit reliability, and error-redaction regressions | Canonical | Global media-type allowlisting is intentionally avoided; route-specific semantic MIME policy and the remaining SSRF/network review remain tracked in #540. |
