...
| Tool/sandbox capability boundary | `docs/TOOL_CAPABILITIES.md`; `skeleton/security/generated_code_sandbox.py`; kernel `skeleton/kernel/sandbox.py` | `tests/test_orchestration.py`; `tests/test_generated_code_sandbox.py`; `skeleton.security.injection_corpus` | Canonical | Generated code remains untrusted data. New tool surfaces must keep deny-by-default grants and fail closed on scanner/parse errors. |
