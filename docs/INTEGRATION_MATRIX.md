...
| Model routing -> capability catalog -> fallback -> eval contract | capable provider is selected in deterministic order with provenance | malformed metadata, timeout, partial failure, and budget exhaustion fail closed without leaking secrets | in-process fake providers; no network | `tests/test_model_routing.py` | `integration_smoke` |
