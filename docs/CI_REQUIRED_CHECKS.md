...
- Security Gate — process, deserialization, SAST, JavaScript process-alias, workflow-security, and repository secret-hygiene scanners.
...
Flaky-test exception rules, security evidence requirements, dependency auditing, and temporary exception expiry are defined in `docs/SECURITY_CI_POLICY.md`. Slow release/load/provenance workflows remain outside the fast merge summary unless their owning policy explicitly promotes them.
