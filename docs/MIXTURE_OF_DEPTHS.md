...
`float("inf")` forces eligible tokens to exit; a negative threshold forces full depth and is useful for calibration or A/B checks. Configuration is deliberately strict: `enabled` must be a real boolean, `min_depth` must be an integer, and thresholds must be real numeric values. Ambiguous or lossy coercions fail fast rather than silently changing routing behavior.
