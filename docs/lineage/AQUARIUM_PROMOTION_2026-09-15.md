...
A parallel aquarium promotion landed while a split model/state implementation was being developed on a stacked branch. The parallel implementation is retained as the canonical domain primitive because it is broader and structurally stronger: it derives fish/decoration totals from canonical tank state, models tank dimensions/themes/visits/likes, preserves source record conversion, and keeps portable policy in one coherent module.

The split duplicate model/state design is deliberately not promoted. Its useful integrity work survives only as a non-duplicative adapter/test layer on top of the canonical module.
...
`aquarium_adapters.py` composes canonical aquarium state with existing frontier primitives and adds no second event bus, memory store, or state model. It provides stable subject identity, canonical state digests, allowlisted `DomainEvent` topics, full event-state rehydration through canonical constructors, and event→memory projection.

Malformed event state is reconstructed through aquarium invariants before its digest is accepted, so recomputing a hash cannot make an invalid materialized state trustworthy.

## Deliberate hardening over source behavior

### Derived counts
The source stores `total_fish_displayed` alongside tank arrays. Frontier totals are derived from materialized tank state, eliminating list/counter drift by construction.
...
### Global identities
Fish identities and placed-decoration identities must be unique across materialized tanks.

### Decoration quantity semantics
The source stores repeated decoration IDs and uses MongoDB `$pull` during placement, which can remove every duplicate match. Frontier inventory is quantity-aware and placement consumes exactly one copy.

### Daily like identity
The source daily-like query becomes a pure `(liker_id, date)` ledger invariant, separated from database persistence.
