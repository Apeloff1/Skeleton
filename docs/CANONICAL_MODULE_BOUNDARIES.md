...
| Model/runtime execution | canonical runtime contract promoted under `skeleton/frontier/` until a dedicated `skeleton/runtime/` package is introduced | provider-neutral request/response, streaming, cancellation, capability negotiation, accounting, capability-based routing, bounded fallback, eval contract | agent policy, HTTP endpoint semantics, vendor fields in core contracts, supervisor/studio planning routes |
