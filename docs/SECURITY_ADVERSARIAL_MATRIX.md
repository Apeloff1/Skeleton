...
The matrix is implementation-neutral: each regression should exercise the production guard or canonical scanner rather than merely assert that a fixture exists.
...
For each newly covered case, the focused test should record the production entry point, malicious input class, expected fail-closed behavior, regression test name, and canonical CI job that executes it.
