...
def test_coordinator_fails_closed_until_ready():
 x,s,d=make(); assert x.admit(0,0,request_id="x") is Admission.SHED; s.ready(); d.mark("db"); assert x.admit(0,0,request_id="x") is Admission.ACCEPT

def test_coordinator_composes_budget_and_rate():
 x,s,d=make(); s.ready(); d.mark("db"); assert x.admit(0,0,request_id="x") is Admission.ACCEPT; assert x.admit(1,0,request_id="y") is Admission.SHED; x.release(); assert x.admit(10,0,request_id="z") is Admission.ACCEPT
...
def test_coordinator_outcomes_drive_circuit_recovery():
 x,s,d=make(threshold=2); s.ready(); d.mark("db"); x.record_outcome(False); assert x.circuit.allowed; x.record_outcome(False); assert not x.circuit.allowed; assert not x.record_outcome(False); assert not x.circuit.allowed; x.circuit.probe(); assert not x.record_outcome(True); assert x.health.value == 0.25; assert x.circuit.allowed
