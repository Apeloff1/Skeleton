/**
 * Computer Science Academy Modal v11.6
 * Comprehensive CS Education for Game Development
 */
...
interface CSAcademyModalProps {
  visible: boolean;
  onClose: () => void;
  colors: any;
}

type Category = 'data_structures' | 'algorithms' | 'systems' | 'graphics' | 'ai_ml' | 'networking' | 'architecture';
type SkillLevel = 'beginner' | 'intermediate' | 'advanced' | 'expert';

export const CSAcademyModal = memo(function CSAcademyModal({
  ...
  const fetchInfo = async () => {
    try {
      const res = await apiFetch(`${API_URL}/api/cs/info`);
      const data = await res.json();
      setInfo(data);
    } catch (e) {
      console.error('Failed to fetch CS info:', e);
    }
  };

  const categories: { id: Category; name: string; icon: string; color: string }[] = [
    { id: 'data_structures', name: 'Data Structures', icon: 'layers', color: '#3B82F6' },
    { id: 'algorithms', name: 'Algorithms', icon: 'git-branch', color: '#8B5CF6' },
    { id: 'systems', name: 'Systems', icon: 'hardware-chip', color: '#10B981' },
    { id: 'graphics', name: 'Graphics', icon: 'image', color: '#F59E0B' },
    { id: 'ai_ml', name: 'AI & ML', icon: 'bulb', color: '#EF4444' },
    { id: 'networking', name: 'Networking', icon: 'wifi', color: '#2563EB' },
    { id: 'architecture', name: 'Architecture', icon: 'construct', color: '#EC4899' },
  ];
  ...
  const learnTopic = async () => {
    ...
    try {
      const res = await apiFetch(`${API_URL}/api/jeeves/teach-cs`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          game_context: selectedCategory ? `Focus on ${selectedCategory} in game development` : null,
          language: 'python',
      ...
      console.error('CS learning error:', e);
    } finally {
      setLoading(false);
    }
  };
        ...
        <View style={[localStyles.modal, { backgroundColor: colors.surface }]}>
          ...
          <View style={[localStyles.header, { borderBottomColor: colors.border }]}>
            <View style={localStyles.headerTitle}>
              ...
              <Text style={[localStyles.title, { color: colors.text }]}>CS Academy</Text>
            </View>
          ...
          <ScrollView style={localStyles.content} showsVerticalScrollIndicator={false}>
            ...
            {info && (
              <View style={[localStyles.infoBanner, { backgroundColor: '#10B98115' }]}>
                <Text style={[localStyles.infoText, { color: colors.text }]}>
                  {info.total_hours} Hours • {info.total_modules} Modules • Full Game Engine Coverage
                </Text>
              </View>
            )}
              ...
              {skillLevels.map((level) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    { backgroundColor: skillLevel === level.id ? '#10B981' : colors.surfaceAlt }
                  ]}
            ...
            <TextInput
              ...
              placeholder="e.g., A* pathfinding, Behavior trees, ECS architecture, Shader programming..."
            ...
            <TouchableOpacity
              style={[localStyles.learnBtn, { backgroundColor: loading ? colors.surfaceAlt : '#10B981' }]}
              onPress={learnTopic}
            ...
            <View style={localStyles.quickTopics}>
              {['A* Pathfinding', 'Behavior Trees', 'ECS', 'Octrees', 'Deferred Rendering', 'Netcode'].map((t) => (
                <TouchableOpacity
                  key={t}
