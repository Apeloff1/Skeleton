...
Cut history:
  - 2026-08-28 (B4b): guarded shim — when the ``skeleton`` package is
    importable, Filler / FillerStore / MemoryWarmer come from the canonical
    ``skeleton.memory.warmer`` (which now owns JSON persistence in the same
    byte format). The backend production image copies only ``backend/``, so
    the ImportError branch (the local implementation) is what runs there
    today; vendoring ``skeleton/`` flips the switch with no code change.
"""
...
from services.cag import CAGPrefix, registry
...
try:  # ── canonical implementation (skeleton on sys.path) ────────────────
    from skeleton.memory.warmer import (
        DEFAULT_TTL_S as _SKELETON_DEFAULT_TTL,
        Filler,
        FillerStore,
        MemoryWarmer,
    )
    ...
    def get_store() -> FillerStore:
        global _store
        if _store is None:
            _store = FillerStore(path=_PERSIST_PATH)
        return _store

    _warmer: MemoryWarmer | None = None

    def get_warmer() -> MemoryWarmer:
        global _warmer
        if _warmer is None:
            _warmer = MemoryWarmer(get_store(), interval_s=_WARMER_INTERVAL_S)
        return _warmer
    ...
    def get_warmer() -> MemoryWarmer:
        global _warmer
        if _warmer is None:
            _warmer = MemoryWarmer(get_store())
        return _warmer
