...
def test_elect_plurality_unchanged():
    winner, n = Voting().elect(
        ["alpha", "beta"],
        [Ballot("a", ("alpha",)), Ballot("b", ("alpha",)), Ballot("c", ("beta",))],
...
def test_elect_approval_unchanged():
    winner, n = Voting().elect(
        ["x", "y", "z"],
        [Ballot("a", ("x", "y")), Ballot("b", ("y", "z")), Ballot("c", ("y",))],
...
def test_cast_and_plurality_tally(diet: Diet):
    ...
    assert [b.voter for b in diet.roll(m.id)] == ["alice", "bob", "cara"]
...
def test_tally_tie_breaks_to_status_quo(diet: Diet):
    ...
    assert diet.tally(m.id)["leading"] == "status-quo"
...
def test_measures_lists_open_and_closed(diet: Diet, clock: Clock):
    ...
    diet.close(a.id)
    clock.advance(MIN_OPEN_SECS + 1)
    assert {m.id for m in diet.measures()} == {a.id, b.id}
