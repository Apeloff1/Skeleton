...
JSON_ENVELOPE_FILES = [
    ...
    REPO_ROOT / "backend" / "services" / "live_scrapers.py",
    REPO_ROOT / "backend" / "services" / "binary_builder.py",
]
...
def test_json_envelopes_do_not_stringify_caught_exceptions(path: Path) -> None:
    ...
    assert "error:{type(exc).__name__" not in source
    assert "toolchain error: {e}" not in source
    assert "type(r).__name__}: {r}" not in source
