...
def test_clean_small_code_still_gets_review_prompt():
    report = CodeIntelligenceEngine().analyze("def add(a, b):\n    return a + b\n")
    assert report.tasks
    assert report.tasks[0].kind in {CodeTaskKind.REVIEW, CodeTaskKind.DOCUMENT}
